...
(function () {
  ...
  function pendingPOForEmployee(employeeId) {
    const prio = { baixa: 0, media: 1, alta: 2 };
    return productionOrders
      .filter(po => po.employeeId === employeeId && po.status !== "finalizada")
      .sort((a, b) => (b.status === "em_producao" ? 2 : b.status === "pausa" ? 1 : 0) - (a.status === "em_producao" ? 2 : a.status === "pausa" ? 1 : 0) ||
  ...
  async function doSaveFinish() {
    ...
    if (po && window.FluxoProducao) {
      try {
        ...
        if (res.proximaIniciada && res.proximaIniciada.tarefa) {
          const next = res.proximaIniciada.tarefa;
          st.productionOrderId = next.id;
          st.orderId = next.orderId;
          st.machId = next.machineId;
          st.startTime = nowHM();
          lsSet("cp_opsession", JSON.stringify({
            machId: next.machineId, productionOrderId: next.id, orderId: next.orderId,
            startTime: st.startTime, prevStatus: "parada", opName: st.opName || "", empId: st.empId || ""
          }));
          st.phase = "running";
          renderAll();
          toast("Produção salva. Próxima tarefa iniciada automaticamente.");
        } else {
          /* Se ainda existe saldo, a tarefa ficou pausada e continuará disponível
             no próximo turno. */
          lsDel("cp_opsession");
          ...
          st.productionOrderId = ""; st.orderId = "";
          renderAll();
          toast(res.terminouTarefa ? "Produção salva" : "Produção do dia salva. A OP continua aberta para o próximo turno.");
        }
        return;
      } catch (e) {
