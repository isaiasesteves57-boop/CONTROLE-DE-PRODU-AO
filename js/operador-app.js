...
(function () {
  ...
  function pendingPOForMachine(machineId) {
    return productionOrders
      .filter(po => po.machineId === machineId && po.status !== "finalizada")
  ...
  function viewOrderReady() {
    ...
    return `
      <div class="op-center">
        <h2 class="op-title">Ordem de produção</h2>
        <p class="op-sub">Confira os dados e inicie a produção</p>
      </div>

      <div class="op-sum">
        <div class="op-sum-item"><div class="sk">Produto</div><div class="sv" style="font-size:14px">${po ? U.esc(po.product) : "—"}</div></div>
  ...
  function viewRunning() {
    ...
    const order = po ? orders.find(x => x.id === po.orderId) : null;
    const label = po ? po.product : (p ? p.name : "");
  ...
  function viewFinish() {
    ...
    const p = po ? null : products.find(x => x.id === st.productId);
    const mins = minsSince(st.startTime);
    const materialLabel = po ? po.product : (p ? p.name + (st.model ? " — " + st.model : "") : "—");
    return `
      <div class="op-center">
  ...
  function onClick(e) {
    ...
    if (a === "begin") {
      if (!st.machId) return;
      /* Se o gestor já criou uma Ordem de Produção para esta máquina,
         o operador recebe essa ordem em vez de escolher o material. */
      const po = pendingPOForMachine(st.machId);
      if (po) {
  ...
  async function confirmOrder() {
    ...
    if (po.status === "aguardando") {
      try { await S.update("productionOrders", po.id, { status: "em_producao", startedAt: Date.now() }); } catch (e) { }
    }
    st.phase = "running";
    renderAll();
  }
  ...
  async function saveFinish() {
    ...
    const rec = {
      ...
      productName: po ? po.product : (p.name || ""),
      model: po ? "" : ((p.hasModels) ? st.model : ""),
    ...
    try { await S.update("machines", st.machId, { status: "parada", client: rec.client || m.client || "" }); } catch (e) { }
    ...
    if (po) {
      ...
      if (newProduced >= (Number(po.quantityPlanned) || 0)) {
        patchPO.status = "finalizada";
        patchPO.finishedAt = Date.now();
      } else if (po.status === "aguardando") {
        patchPO.status = "em_producao";
      }
      try { await S.update("productionOrders", po.id, patchPO); } catch (e) { }

      if (order && po.operacao === "producao") {
        const mainPOs = productionOrders.filter(x => x.orderId === order.id && x.operacao === "producao");
        const totalProduced = mainPOs.reduce((s, x) => s + (x.id === po.id ? newProduced : (Number(x.quantityProduced) || 0)), 0);
        const patchOrder = {};
        if (totalProduced >= (Number(order.quantity) || 0)) {
        ...
        if (Object.keys(patchOrder).length) {
          try { await S.update("orders", order.id, patchOrder); } catch (e) { }
        }
      }
    }
