...
(function () {
  ...
  /* Catálogo de Produtos por código (Etapa 12): usado para achar o
     produto/pedido a partir do código falado no Registro por Voz. */
  let skus = [];

  /* Registro de Produção por Voz sem Tarefa pré-atribuída (Etapa 12):
     estado isolado do resto da tela, para não interferir no fluxo
     de Tarefa/OP já existente. */
  function freshVr() {
    return {
      listening: false, rawText: "", clienteRaw: "", codigoRaw: "",
      qty: "", def: "0", sku: null, candidates: [], orderId: "",
      err: "", saving: false
    };
  }
  ...
  const st = {
    ...
    def: "",
    finishErr: "",
    /* Comando por Voz (Etapa 8): true enquanto o microfone está ouvindo. */
    ...
    productionOrderId: "",
    orderId: "",
    /* Registro de Produção por Voz (Etapa 12). */
    vr: freshVr()
  };
  let timer = null;

  /* Comparação tolerante (Etapa 8): usadas só para conferir, sem
     bloquear, se o cliente/código falados batem com a tarefa atual. */
  function normTextLocal(s) {
    return String(s || "").trim().toLowerCase()
      .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
      .replace(/\s+/g, " ");
  }
  function normCodeLocal(s) {
  ...
  /* ---------- REGISTRO DE PRODUÇÃO POR VOZ, SEM TAREFA (Etapa 12) ----------
     Localiza sozinho o Pedido em aberto a partir do código (catálogo)
     e do cliente falados — sem depender de uma Tarefa que o gestor já
     tenha atribuído. Usa as mesmas comparações tolerantes (acento/caixa)
     já usadas na conferência do comando de voz no fim de turno. */
  function findSkuByCodeLocal(code) {
    if (!code) return null;
    const n = normCodeLocal(code);
    return skus.find(s => normCodeLocal(s.code) === n) || null;
  }

  function pedidosAbertosPara(sku, clienteRaw) {
    let list = orders.filter(o => o.status !== "finalizado");
    if (sku) list = list.filter(o => o.skuId === sku.id || normCodeLocal(o.skuCode) === normCodeLocal(sku.code));
    if (clienteRaw) {
      const alvo = normTextLocal(clienteRaw);
      const porCliente = list.filter(o =>
        normTextLocal(o.client).indexOf(alvo) >= 0 || alvo.indexOf(normTextLocal(o.client)) >= 0);
      if (porCliente.length) list = porCliente;
    }
    return list.slice().sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
  }

  function matchOrderFromVoice() {
    const sku = findSkuByCodeLocal(st.vr.codigoRaw);
    st.vr.sku = sku;
    const candidates = pedidosAbertosPara(sku, st.vr.clienteRaw);
    st.vr.candidates = candidates;
    st.vr.orderId = candidates.length ? candidates[0].id : "";
  }
    ...
    if (sess && sess.machId && (sess.productionOrderId || sess.productId)) {
      ...
      if (sess.productionOrderId) {
        if (machineOk && productionOrders.some(po => po.id === sess.productionOrderId)) {
          ...
          const sessPo = productionOrders.find(x => x.id === sess.productionOrderId);
          st.phase = (sessPo && sessPo.status === "pausa") ? "paused" : "running";
  ...
  function softRender() {
    if (st.phase === "running" || st.phase === "finish" || st.phase === "voice-registro") return;
    ...
    root.innerHTML = `
      <div class="op-app">
        <div class="op-head">
          ...
          ${identified
            ? (() => { const em = employees.find(x => x.id === st.empId); return `<div class="op-user"><div class="op-avatar">${em && em.photoData ? `<img src="${U.esc(em.photoData)}" alt="Foto">` : U.icon("user", "sm")}</div><div class="tiny dim" style="text-align:right">${U.esc(st.opName)}</div></div>`; })()
  ...
  function phaseView() {
    ...
    if (st.phase === "paused") return viewPaused();
    if (st.phase === "finish") return viewFinish();
    if (st.phase === "voice-registro") return viewVoiceRegistro();
    ...
    return `
      <div class="op-center">
        <div class="op-profile">
          <div class="op-avatar xl">${em && em.photoData ? `<img src="${U.esc(em.photoData)}" alt="Foto">` : U.icon("user", "lg")}</div>
      ...
      ${window.ComandoVoz && window.ComandoVoz.suportado() ? `
      <button class="op-big ghost" data-act="voice-registro-start" ${st.machId ? "" : "disabled"}>🎙 Registrar Produção por voz</button>
      <p class="op-sub" style="margin:2px 0 0">Fale, por exemplo: "Cliente Papelaria, código AG C BCP 27, produzi 500 unidades, 10 defeitos"</p>` : ""}` : ""}
  ...
  function viewRunning() {
    ...
    const info = po ? pedidoInfo(po.orderId) : null;
    const planned = po ? (Number(po.plannedQuantity) || 0) : 0;
    const produced = po ? (Number(po.quantityProduced) || 0) : 0;
    const restTarefa = Math.max(0, planned - produced);
    return `
      <div class="op-center">
        <span class="op-run-led"></span>
        <h2 class="op-title" style="color:var(--ok);margin:8px 0 2px">MINHA PRODUÇÃO</h2>
      ...
      ${po ? `<div class="op-sum">
        <div class="op-sum-item"><div class="sk">Planejado</div><div class="sv">${planned ? U.fmt(planned) : "—"}</div></div>
        <div class="op-sum-item"><div class="sk">Produzido</div><div class="sv">${U.fmt(produced)}</div></div>
        <div class="op-sum-item"><div class="sk">Restante</div><div class="sv" style="color:var(--amber2)">${planned ? U.fmt(restTarefa) : "—"}</div></div>
      </div>` : ""}
      ...
      <button class="op-big warn" data-act="pause-run">${U.icon("pause")} PAUSAR</button>
      <button class="op-big safe" data-act="open-finish">${U.icon("checkc")} FINALIZAR PRODUÇÃO</button>
      <button class="op-big ghost" data-act="leave-running">${U.icon("logout")} SAIR E VOLTAR DEPOIS</button>
      <p class="tiny dim" style="text-align:center;margin-top:8px">Ao finalizar, informe a quantidade produzida e os defeitos — a baixa da Tarefa, da OP e do Pedido acontece automaticamente.</p>`;
  }
  ...
  function viewPaused() {
    ...
    return `
      <div class="op-center">
        <h2 class="op-title" style="color:var(--amber2);margin:8px 0 2px">PRODUÇÃO PAUSADA</h2>
        <p class="op-sub">${m ? U.esc(m.name) : ""} · ${U.esc(label)}</p>
      </div>

      <div class="op-card op-center">
        <div class="op-clock" id="op-clock">${fmtClock(mins * 60)}</div>
        <div class="op-run-info mt8">
          <span class="op-clock-start">Pausada — o tempo pausado não conta na produção</span>
        </div>
      </div>

      <button class="op-big go" data-act="resume-run">${U.icon("play")} RETOMAR</button>
      <button class="op-big safe" data-act="open-finish">${U.icon("checkc")} FINALIZAR PRODUÇÃO</button>
      <button class="op-big ghost" data-act="leave-running">${U.icon("logout")} SAIR E VOLTAR DEPOIS</button>`;
  }
  ...
  function viewFinish() {
    ...
    return `
      ...
      ${window.ComandoVoz && window.ComandoVoz.suportado() ? `
      <button class="op-big ghost" data-act="voice-finish" style="${st.voiceListening ? "opacity:.6;pointer-events:none" : ""}">
        ${st.voiceListening ? "🎙️ Ouvindo..." : "🎤 Preencher por voz"}
      </button>
      <p class="op-sub" style="margin:2px 0 12px">Ex.: "Cliente Papelaria, código AG C BCP 27, produzi 500 unidades, 10 defeitos"</p>` : ""}
  ...
  /* ---------- REGISTRO DE PRODUÇÃO POR VOZ (Etapa 12) ----------
     Tela isolada: fala → acha pedido/produto sozinho → confere/edita →
     salva. Reaproveita FluxoProducao.registrarProducaoPorPedido, que
     por sua vez reaproveita iniciar()/finalizar() já existentes. */
  function viewVoiceRegistro() {
    const m = machines.find(x => x.id === st.machId);
    const vr = st.vr;
    const hasResult = !!vr.rawText;
    const order = vr.orderId ? orders.find(o => o.id === vr.orderId) : null;
    const info = vr.orderId ? pedidoInfo(vr.orderId) : null;
    const qtyNum = Number(vr.qty) || 0, defNum = Number(vr.def) || 0;
    const aprovado = Math.max(0, qtyNum - defNum);

    return `
      <div class="op-center">
        <h2 class="op-title">Registrar Produção por voz</h2>
        <p class="op-sub">Máquina: <b style="color:var(--amber2)">${m ? U.esc(m.name) : "—"}</b></p>
      </div>

      ${!hasResult ? `
      <div class="op-card">
        <p class="op-sub mb8">Toque e fale, por exemplo:</p>
        <p class="op-sub" style="font-style:italic">"Cliente Papelaria, código AG C BCP 27, produzi 500 unidades, 10 defeitos"</p>
      </div>
      <button class="op-big go" data-act="vr-listen" style="${vr.listening ? "opacity:.6;pointer-events:none" : ""}">
        ${vr.listening ? "🎙️ Ouvindo..." : "🎤 Falar agora"}
      </button>
      ${vr.err ? `<p class="op-pin-err">${U.esc(vr.err)}</p>` : ""}
      <button class="op-big ghost" data-act="vr-cancel">Cancelar</button>` : `

      <div class="op-card">
        <p class="op-sub mb8">Você falou</p>
        <p class="op-sub" style="font-style:italic">"${U.esc(vr.rawText)}"</p>
      </div>

      <div class="op-sum">
        <div class="op-sum-item"><div class="sk">Cliente falado</div><div class="sv" style="font-size:14px">${vr.clienteRaw ? U.esc(vr.clienteRaw) : "—"}</div></div>
        <div class="op-sum-item"><div class="sk">Código falado</div><div class="sv" style="font-size:14px">${vr.codigoRaw ? U.esc(vr.codigoRaw) : "—"}</div></div>
      </div>
      <div class="op-card">
        <p class="op-sub">${vr.err ? "O comando não foi salvo. Nenhum dado foi alterado." : "Validando e registrando automaticamente…"}</p>
        ${vr.err ? `<p class="op-pin-err">${U.esc(vr.err)}</p><p class="op-sub mt8">Repita incluindo cliente, código, quantidade e, se houver mais de um pedido, o número do pedido.</p>` : ""}
      </div>
      <button class="op-big go" data-act="vr-listen" style="${vr.listening ? "opacity:.6;pointer-events:none" : ""}">${vr.listening ? "🎙️ Ouvindo..." : "🎤 Falar novamente"}</button>
      <button class="op-big ghost" data-act="vr-cancel">Cancelar</button>`}`;
  }
  ...
  function wire() {
    ...
    const vrOrderEl = document.getElementById("vr-order");
    if (vrOrderEl) vrOrderEl.addEventListener("change", () => { st.vr.orderId = vrOrderEl.value; st.vr.err = ""; renderAll(); });
    const vrQtyEl = document.getElementById("vr-qty");
    if (vrQtyEl) vrQtyEl.addEventListener("input", () => { st.vr.qty = vrQtyEl.value; st.vr.err = ""; });
    const vrDefEl = document.getElementById("vr-def");
    if (vrDefEl) vrDefEl.addEventListener("input", () => { st.vr.def = vrDefEl.value; st.vr.err = ""; });
  }
  ...
  function onClick(e) {
    ...
    if (a === "begin") {
    ...
    else if (a === "pause-run") pauseRun();
    else if (a === "resume-run") resumeRun();
    ...
    else if (a === "voice-finish") voiceFillFinish();
    else if (a === "voice-registro-start") {
      if (!st.machId) return;
      st.vr = freshVr();
      st.phase = "voice-registro";
      renderAll();
    }
    else if (a === "vr-listen") vrListen();
    else if (a === "vr-save") vrSalvar();
    else if (a === "vr-cancel") {
      st.vr = freshVr();
      ...
      st.productionOrderId = ""; st.orderId = ""; st.vr = freshVr();
      ...
      st.empId = ""; st.opName = ""; st.machId = ""; st.empPin = ""; st.vr = freshVr();
      lsDel("cp_opname");
      st.phase = "pin-login";
      renderAll();
    }
  ...
  /* Início de turno vinculado a uma Ordem de Produção (Módulo de pedidos):
     ...
     A partir da Etapa 8, o início passa pelo FluxoProducao, que também
     marca a máquina como "produzindo" e a OP como "producao". */
  async function confirmOrder() {
    ...
    try {
      if (window.FluxoProducao) {
        await FluxoProducao.iniciarTarefa({ taskId: po.id });
      } else {
        await S.update("machines", st.machId, {
          status: "produzindo",
        ...
        if (po.status === "aguardando") {
          await S.update("productionOrders", po.id, { status: "em_producao", startedAt: Date.now() });
        }
      }
    } catch (e) { toast(errMsg(e)); renderAll(); return; }
    st.phase = "running";
    renderAll();
  }

  /* Pausa/retomada da produção (Etapa 8): o tempo pausado não conta no
     cálculo da eficiência — o FluxoProducao acumula pausedSeconds. */
  async function pauseRun() {
    if (st.productionOrderId) {
      try {
        if (window.FluxoProducao) await FluxoProducao.pausarTarefa({ taskId: st.productionOrderId });
        else await S.update("machines", st.machId, { status: "pausa" });
      } catch (e) { toast(errMsg(e)); return; }
    } else {
      try { await S.update("machines", st.machId, { status: "pausa" }); } catch (e) { toast(errMsg(e)); return; }
    }
    st.phase = "paused";
    renderAll();
  }
  ...
  async function resumeRun() {
    if (st.productionOrderId) {
      try {
        if (window.FluxoProducao) await FluxoProducao.retomarTarefa({ taskId: st.productionOrderId });
        else await S.update("machines", st.machId, { status: "produzindo" });
      } catch (e) { toast(errMsg(e)); }
    } else {
      try { await S.update("machines", st.machId, { status: "produzindo" }); } catch (e) { toast(errMsg(e)); }
    }
    st.phase = "running";
    st.startTime = st.startTime || nowHM();
    renderAll();
  }
  ...
  /* ========== COMANDO POR VOZ (Etapa 17) ==========
     A frase reconhecida não preenche formulário manual: mostra apenas um
     resumo operacional para confirmação. Só depois da confirmação a
     AutomacaoFinal valida a tarefa, registra produção, atualiza estoque e
     grava a trilha de auditoria. */
  function confirmarApontamentoVoz(parsed) {
    const p = parsed || {};
    const qty = Number(p.quantidade) || 0;
    const defects = Number(p.defeitos) || 0;
    const approved = Math.max(0, qty - defects);
    const cliente = p.clienteRaw ? "\nCliente: " + p.clienteRaw : "";
    const codigo = p.codigoRaw ? "\nCódigo: " + p.codigoRaw : "";
    const pedido = p.numeroPedido ? "\nPedido: " + p.numeroPedido : "";
    return window.confirm("Confirmar apontamento?" + cliente + codigo + pedido +
      "\nProdução: " + qty + " unidades" +
      "\nDefeitos: " + defects +
      "\nAprovado/estoque: " + approved + " unidades");
  }

  async function voiceFillFinish() {
    if (!window.ComandoVoz || !window.ComandoVoz.suportado()) {
      toast("Reconhecimento de voz não é compatível com este navegador.");
      return;
    }
    if (!window.AutomacaoFinal || !window.FluxoProducao) {
      toast("Automação por voz indisponível. Recarregue a tela.");
      return;
    }
    const po = st.productionOrderId ? productionOrders.find(x => x.id === st.productionOrderId) : null;
    if (!po) {
      toast("Para a produção livre, use Registrar Produção por voz e informe cliente e código.");
      return;
    }
    st.voiceListening = true;
    renderAll();
    try {
      const texto = await window.ComandoVoz.ouvir();
      const r = window.ComandoVoz.parseComando(texto);
      st.voiceListening = false;
      if (!confirmarApontamentoVoz(r)) throw new Error("Apontamento cancelado pelo operador.");
      const res = await window.AutomacaoFinal.apontarPorVoz({
        parsed: r, currentTaskId: po.id, machineId: st.machId,
        employeeId: st.empId || "", employeeName: st.opName || "Operador"
      });
      const rec = res.rec || {};
      st.qty = ""; st.def = ""; st.finishErr = "";
      if (res.concluida) {
        lsDel("cp_opsession");
        st.lastDone = { machineName: rec.machineName, productName: rec.productName, qty: r.quantidade };
        st.productionOrderId = ""; st.orderId = ""; st.phase = "done";
      } else {
        st.phase = "running";
      }
      toast("Apontamento por voz registrado automaticamente.");
      renderAll();
    } catch (e) {
      st.voiceListening = false;
      st.finishErr = (e && e.message) || "Não foi possível registrar o comando por voz.";
      renderAll();
    }
  }

  /* Ouve a frase, localiza um único pedido aberto e gera o apontamento.
     Em caso de pedido ambíguo ou inválido, nada é salvo e o operador pode
     repetir a frase incluindo o número do pedido. */
  async function vrListen() {
    if (!window.ComandoVoz || !window.ComandoVoz.suportado()) {
      toast("Reconhecimento de voz não é compatível com este navegador.");
      return;
    }
    st.vr.listening = true;
    st.vr.err = "";
    renderAll();
    try {
      const texto = await window.ComandoVoz.ouvir();
      const r = window.ComandoVoz.parseComando(texto);
      st.vr.listening = false;
      st.vr.rawText = texto;
      st.vr.clienteRaw = r.clienteRaw || "";
      st.vr.codigoRaw = r.codigoRaw || "";
      st.vr.qty = r.quantidade !== "" ? String(r.quantidade) : "";
      st.vr.def = r.defeitos !== "" ? String(r.defeitos) : "0";
      matchOrderFromVoice();
      if (!window.AutomacaoFinal) throw new Error("Automação por voz indisponível. Recarregue a tela.");
      if (!confirmarApontamentoVoz(r)) throw new Error("Apontamento cancelado pelo operador.");
      const res = await window.AutomacaoFinal.apontarPorVoz({
        parsed: r, machineId: st.machId, employeeId: st.empId || "",
        employeeName: st.opName || "Operador"
      });
      const rec = res.rec || {};
      st.lastDone = { machineName: rec.machineName, productName: rec.productName, qty: r.quantidade };
      st.vr = freshVr();
      ...
      st.vr.listening = false;
      st.vr.err = (e && e.message) || "Não foi possível registrar o comando.";
      renderAll();
    }
  }

  async function vrSalvar() {
    const vr = st.vr;
    if (vr.saving) return;
    const orderId = vr.orderId;
    const qty = Number(vr.qty) || 0;
    const defects = Number(vr.def) || 0;
    if (!orderId) { vr.err = "Selecione o pedido antes de salvar."; renderAll(); return; }
    if (qty <= 0) { vr.err = "Informe a quantidade produzida."; renderAll(); return; }
    if (defects < 0) { vr.err = "Defeitos inválidos."; renderAll(); return; }
    const info = pedidoInfo(orderId);
    if (info && qty > info.remaining) {
      vr.err = `A quantidade informada é maior que o saldo disponível deste pedido. Restam apenas ${U.fmt(info.remaining)} unidades.`;
      renderAll();
      return;
    }
    vr.saving = true;
    renderAll();
    try {
      const res = await window.FluxoProducao.registrarProducaoPorPedido({
        orderId: orderId, qty: qty, defects: defects,
        machineId: st.machId, employeeId: st.empId || "",
        employeeName: st.opName || "Operador",
        rawText: vr.rawText || ""
      });
      st.lastDone = { machineName: res.rec.machineName, productName: res.rec.productName, qty: qty };
      st.vr = freshVr();
      st.phase = "done";
      renderAll();
  ...
  async function doSaveFinish() {
    ...
    /* Etapa 8: quando a produção está vinculada a uma Tarefa, a baixa
       automática (Tarefa → OP → Pedido) fica centralizada no
       FluxoProducao — o tempo pausado também é descontado. */
    if (po && window.FluxoProducao) {
      try {
        const res = await FluxoProducao.finalizarTarefa({ taskId: po.id, qty: qty, defects: def });
        const rec = res.rec;
        lsDel("cp_opsession");
