...
(function () {
  ...
  async function registrarProducaoParcial(opts) {
    ...
    const rec={
      ...
      quantityProduced:qty, defects:defects, productionHours:hours,
    ...
    if(complete) await S.update("machines",po.machineId,{status:"parada"});
    await baixaAutomatica(po.id);
    if(complete && po.employeeId) await iniciarProximaFuncionario(po.employeeId);
