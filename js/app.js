...
(function () {
  ...
  try {
    ...
    if (["dashboard", "lance", "relatorios", "planejamento", "maquinas", "produtos", "config"].indexOf(h) >= 0) state.page = h;
  } catch (e) { }
  ...
  function renderConfig(v) {
    ...
    v.innerHTML = `
      ...
      <div class="card pad mt12">
        ...
        <p class="small dim">Versão: <b style="color:var(--text)">cp-v3</b> · PWA instalável no Android</p>
  ...
  function onClickGlobal(e) {
    ...
    if (a === "login") doLogin();
      ...
      ["cp_config", "cp_machines", "cp_products", "cp_productions"].forEach(k => { try { localStorage.removeItem(k); } catch (er) { } });
      location.reload();
    }
  }
