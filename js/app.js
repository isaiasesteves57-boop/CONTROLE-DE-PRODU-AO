...
(function () {
  ...
  let machines = [], products = [], productions = [], employees = [], orders = [], productionOrders = [], materials = [], stockMovements = [], config = SEED_CONFIG[0];
  ...
  try {
    ...
    if (["dashboard", "lance", "relatorios", "planejamento", "pedidos", "estoque", "maquinas", "produtos", "funcionarios", "config"].indexOf(h) >= 0) state.page = h;
  } catch (e) { }
  ...
  */
  ...
  function onClickGlobal(e) {
    ...
    if (a === "login") doLogin();
      ...
      ["cp_config", "cp_machines", "cp_products", "cp_productions", "cp_employees", "cp_orders", "cp_productionOrders", "cp_materials", "cp_stockMovements"].forEach(k => { try { localStorage.removeItem(k); } catch (er) { } });
      location.reload();
    }
  }
