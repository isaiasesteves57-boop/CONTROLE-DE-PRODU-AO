...
(function () {
  ...
  // Correção Produto v2: garante carregamento da lista suspensa mesmo
  // quando a descrição vier com diferença de maiúsculas/minúsculas.
  function productsByDescription(description) {
    const key = String(description || "").trim();
    const found = Object.keys(ORDER_PRODUCTS_BY_DESCRIPTION)
      .find(k => k.toLowerCase() === key.toLowerCase());
    return found ? ORDER_PRODUCTS_BY_DESCRIPTION[found] : [];
  }
  const freshOrderDraft = () => ({ number: "", client: "", description: "", product: "", type: "", model: "", finish: "", quantity: "" });
  ...
  */
  ...
  function orderCard(o) {
    ...
    if (d) {
      const allowed = productsByDescription(d.description);
      return `
    ...
    return `
      <div class="card pad"><div class="rowc"><div class="f1">
        ...
        <div class="tiny dim">${o.description?U.esc(o.description)+" · ":""}${U.esc(o.product)}${o.type?" · "+U.esc(o.type):""}${o.model?" · "+U.esc(o.model):""}${o.finish?" · "+U.esc(o.finish):""}</div></div>
  ...
  function newOrderForm() {
    ...
    const allowedProducts = productsByDescription(no.description);
  ...
  function bindAllExtra() {
    ...
    ["no-client", "no-number", "no-product", "no-model", "no-quantity", "no-type", "no-finish"].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
  ...
  async function addOrder() {
    ...
    if (!description || !product || !type || !model || !finish || quantity <= 0) {
      toast("Preencha descrição, produto, tipo, modelo, acabamento e uma quantidade maior que zero.");
      return;
    }
  ...
  async function saveOrder(id) {
    ...
    const patch={client:String(d.client||"").trim(),number:String(d.number||"").trim(),description,product,type:String(d.type||"").trim(),model:String(d.model||"").trim(),finish:String(d.finish||"").trim(),quantity:Number(d.quantity)||0};
    const allowed=productsByDescription(description);
    if(!patch.client||patch.quantity<=0||!description||!allowed.includes(product)||!ORDER_TYPES.includes(patch.type)||!ORDER_MODELS.includes(patch.model)||!ORDER_FINISHES.includes(patch.finish)){toast("Revise os campos do pedido. Produto deve ser compatível com a descrição.");return;}
  ...
  function onClickGlobal(e) {
    ...
    if (a === "login") doLogin();
      ...
      if (o) state.draftOrder[id] = {
        ...
        type: o.type || "", model: o.model || "", finish: o.finish || "", quantity: o.quantity, status: o.status || "aguardando"
      };
      renderView();
    }
