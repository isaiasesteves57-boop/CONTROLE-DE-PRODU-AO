...
(function () {
  ...
  /* Cadastro de Clientes (Etapa 2): usado para selecionar o cliente no
     Pedido em vez de digitar texto livre. Começa com os 3 clientes
     informados; o gestor pode cadastrar, editar e excluir outros. */
  const SEED_CLIENTS = [
    { id: "cliente-leitura", name: "Leitura" },
    { id: "cliente-papelaria", name: "Papelaria" },
    { id: "cliente-varejo", name: "Varejo" }
  ];
  /* Catálogo de Produtos por código (Etapa 3): entidade NOVA e separada
     do cadastro "Produtos" já existente (que representa o modelo de
     furação vinculado à máquina, sem código/estoque). Aqui o código é a
     chave usada para localizar o item (ex.: leitura de pedido, comando
     por voz). Começa vazio — o gestor cadastra os itens da fábrica. */
  const SEED_SKUS = [];
  const SEED_PRODUCTS = [
  ...
  const SEED_OPS = [];
  const ORDER_DESCRIPTIONS = ["Caderno", "Agenda", "Bloco", "Refil"];
  const ORDER_PRODUCTS = ["Capas", "Contra Capas", "Miolo", "Bolsa", "Encarte"];
  const ORDER_TYPES = ["Clássico", "Médio", "Universitário", "Executivo"];
  const ORDER_MODELS = ["Espiral", "Wiro", "Smart", "Fichário"];
  const ORDER_FINISHES = ["Escanteado", "Reto"];
  /* Quantidade de folhas do pedido — lista fixa de opções. */
  const ORDER_SHEET_OPTIONS = [80, 100, 140, 160, 200];
  const ORDER_PRODUCTS_BY_DESCRIPTION = {
    "Caderno": ORDER_PRODUCTS.slice(),
    "Agenda": ORDER_PRODUCTS.slice(),
    "Bloco": ORDER_PRODUCTS.slice(),
    "Refil": ["Miolo", "Encarte"]
  };
  const freshOrderDraft = () => ({ number: "", client: "", skuCode: "", description: "", product: "", sheets: "", type: "", model: "", finish: "", quantity: "" });
  ...
  let machines = [], products = [], productions = [], employees = [], orders = [], productionOrders = [], ops = [], materials = [], stockMovements = [], defectRecords = [], auditLogs = [], clients = [], skus = [], config = SEED_CONFIG[0];
  ...
  const state = {
    ...
    draftClient: {},
    newClientForm: false,
    newClient: { name: "", aliases: "" },
    /* Catálogo de Produtos por código / SKU (Etapa 3) */
    draftSku: {},
    newSkuForm: false,
    newSku: { code: "", description: "", category: "", unit: "UN", stock: 0 },
    draftEmployee: {},
    newEmployeeForm: false,
    selectedEmployeeId: "",
    selectedOPId: "",
    newEmployee: { name: "", role: "", pin: "", pin2: "", status: "ativo", accessLevel: "operador", allowedMachines: [], photoData: "" },
    pin: "",
    err: "",
    /* Pedidos e Ordens de Produção */
    newOrderForm: false,
    /* Etapa 6: quando true, mostra a telinha resumida "Confirmar pedido"
       (Cliente/Produto/Quantidade) em vez do formulário completo — usada
       logo após a Leitura de Pedido por foto/PDF. */
    newOrderConfirmMode: false,
  ...
  try {
    ...
    if (["dashboard", "industria", "relatorios", "planejamento", "pedidos", "ops", "fila", "estoque", "produtividade", "qualidade", "maquinas", "produtos", "clientes", "catalogo", "funcionarios", "config"].indexOf(h) >= 0) state.page = h;
  } catch (e) { }
  ...
  const PAGES = [
    ...
    ["catalogo", "Catálogo de Produtos", "box", "cadastros"],
    ["clientes", "Clientes", "user", "cadastros"],
    ["funcionarios", "Funcionários", "user", "cadastros"],
    ["historico", "Histórico de alterações", "clock", "sistema"],
  ...
  const TITLES = {
    ...
    catalogo: ["Catálogo de Produtos", "Cadastro por código (SKU), descrição, categoria, unidade e estoque atual"],
    clientes: ["Clientes", "Cadastro de clientes"],
    funcionarios: ["Funcionários", "Cadastro, PIN e permissões de acesso"],
    historico: ["Histórico de alterações", "Auditoria de pedidos e lançamentos"],
  ...
  function init() {
    ...
    S.on("auditLogs", d => { auditLogs = d; softRender(); });
    S.on("clients", d => { clients = d; softRender(); });
    S.on("skus", d => { skus = d; softRender(); });
    ...
    S.init("auditLogs", []);
    S.init("clients", SEED_CLIENTS);
    S.init("skus", SEED_SKUS);
      ...
      <div class="shell">
        <aside class="side">
          ...
          <nav class="side-nav">
            ...
            ${sideItem("catalogo", "Catálogo de Produtos", "box")}
            ${sideItem("clientes", "Clientes", "user")}
  ...
  function sheetHTML() {
    const extra = [
      ...
      ["catalogo", "Catálogo de Produtos", "box"],
      ["clientes", "Clientes", "user"],
      ["funcionarios", "Funcionários", "user"],
      ["historico", "Histórico de alterações", "clock"],
  ...
  function renderView() {
    ...
    else if (state.page === "catalogo") renderCatalogo(v);
    else if (state.page === "clientes") renderClientes(v);
    else if (state.page === "funcionarios") renderFuncionarios(v);
    else if (state.page === "funcionario-detalhe") renderFuncionarioDetalhe(v);
    else if (state.page === "historico") renderHistorico(v);
  ...
  /* Etapa 11 — Dashboard gerencial: agregado do MÊS (ano-mês da data do
     painel), no mesmo padrão de dayAgg — só soma o que já está salvo em
     "productions", nada de contador separado. */
  function monthAgg(date) {
    const ym = String(date || U.todayStr()).slice(0, 7); // "AAAA-MM"
    const month = productions.filter(p => String(p.date || "").slice(0, 7) === ym);
    ...
    const defectPct = total ? defects / total * 100 : 0;
    return { ym, total, defects, defectPct, count: month.length };
  }
  ...
  function renderDashboard(v) {
    ...
    const meta = metaRaw > 0 ? metaRaw : avgDailyProd();
    const a = dayAgg(date);
    const mAgg = monthAgg(date);
    const pedidosAndamento = orders.filter(o => o.status === "aguardando" || o.status === "em_producao").length;
    ...
    v.innerHTML = `
      ...
      <div class="grid-cards">
        ...
        <div class="card stat">
          <div class="lbl">${U.icon("calendar", "sm")} Produção do mês</div>
          <div class="val amb">${U.fmt(mAgg.total)}<em>un.</em></div>
          <div class="foot">${mAgg.total ? U.fmt(mAgg.defectPct, 2) + "% de defeitos no mês" : "sem produção no mês"}</div>
        </div>
        <div class="card stat" data-go="pedidos" style="cursor:pointer">
          <div class="lbl">${U.icon("clipboard", "sm")} Pedidos em andamento</div>
          <div class="val">${U.fmt(pedidosAndamento)}</div>
          <div class="foot">aguardando ou em produção</div>
        </div>
        <div class="card stat" data-go="pedidos" style="cursor:pointer">
          <div class="lbl">${U.icon("checkc", "sm")} Pedidos finalizados</div>
          <div class="val up">${U.fmt(pedidosFinalizados)}</div>
          <div class="foot">de ${U.fmt(orders.length)} pedidos no total</div>
        </div>
      </div>
      <div class="card pad mt12 stock-dashboard">
        ...
        <div class="between mt12" style="padding-top:10px;border-top:1px solid var(--border)">
          <span class="tiny dim" style="font-weight:600">Catálogo de produtos (estoque automático)</span>
          <button class="btn-ghost" data-go="catalogo">Abrir catálogo</button>
        </div>
        <div class="stock-dashboard-grid mt8">
          <div><span>Produtos cadastrados</span><b>${U.fmt(skus.length)}</b></div>
          <div><span>Estoque total</span><b>${U.fmt(skus.reduce((s,x)=>s+(Number(x.stock)||0),0))}</b></div>
          <div><span>Sem estoque</span><b>${U.fmt(skus.filter(x=>(Number(x.stock)||0)<=0).length)}</b></div>
        </div>
      </div>
  ...
  */
  ...
  async function addStockMaterial() {
    ...
    try {
      const saved = await S.add("materials",{ name, minStock, productId:state.newMaterial.productId||"", active:true, createdAt:Date.now() });
      await audit("CRIADO", "Material", saved.id, null, saved, "Material físico cadastrado");
  ...
  async function addStockMovement() {
    ...
    try {
      const savedMovement = await S.add("stockMovements",{materialId:n.materialId,quantity:qty,date:n.date,type:n.type,orderId:n.orderId||"",observation:String(n.observation||"").trim(),createdAt:Date.now(),source:"gestor"});
      await audit("CRIADO", "Movimentação de estoque", savedMovement.id, null, savedMovement, "Movimentação de estoque registrada pelo gestor");
  ...
  function aprovadoProducao(row) {
    return Math.max(0, (Number(row && row.quantityProduced) || 0) - (Number(row && row.defects) || 0));
  }

  /* Ajusta o estoque do SKU quando um apontamento é corrigido/excluído.
     O lançamento original permanece no histórico; este movimento de ajuste
     torna o saldo físico auditável e evita que uma correção deixe o produto
     acabado superestimado. */
  async function ajustarEstoqueSkuPorProducao(before, after, note) {
    const beforeOrder = before && before.orderId ? orders.find(o => o.id === before.orderId) : null;
    const afterOrder = after && after.orderId ? orders.find(o => o.id === after.orderId) : null;
    const beforeSkuId = beforeOrder && beforeOrder.skuId;
    const afterSkuId = afterOrder && afterOrder.skuId;
    const deltas = {};
    if (beforeSkuId && before.operacao !== "retrabalho") deltas[beforeSkuId] = (deltas[beforeSkuId] || 0) - aprovadoProducao(before);
    if (afterSkuId && after.operacao !== "retrabalho") deltas[afterSkuId] = (deltas[afterSkuId] || 0) + aprovadoProducao(after);
    for (const skuId of Object.keys(deltas)) {
      const delta = deltas[skuId];
      if (!delta) continue;
      const sku = skus.find(s => s.id === skuId);
      if (!sku) continue;
      const nextStock = Math.max(0, (Number(sku.stock) || 0) + delta);
      await S.update("skus", sku.id, { stock: nextStock });
      const movement = await S.add("stockMovements", {
        materialId: "", skuId: sku.id, productId: sku.id,
        productCode: sku.code || "", productName: sku.description || "",
        type: "AJUSTE_PRODUCAO", quantity: delta,
        date: U.todayStr(), time: new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }),
        orderId: (afterOrder || beforeOrder || {}).id || "",
        productionId: (after || before || {}).id || "",
        observation: note || "Ajuste automático após correção de produção",
        createdAt: Date.now(), source: "production-correction"
      });
      await audit("AJUSTE_AUTOMATICO", "Estoque de produto", sku.id, sku,
        Object.assign({}, sku, { stock: nextStock }), note || "Estoque ajustado após correção de produção");
      await audit("CRIADO", "Movimentação de estoque", movement.id, null, movement, "Ajuste de estoque derivado de correção de produção");
    }
  }
  ...
  async function excluirProducao(id) {
    const before = productions.find(x => x.id === id);
    try { await S.remove("productions", id); } catch (e) { toast(errMsg(e)); return; }
    try { await ajustarEstoqueSkuPorProducao(before, null, "Estorno automático após exclusão de apontamento"); } catch (e) { toast(errMsg(e)); }
    await audit("EXCLUÍDO", "Lançamento", id, before, null, "Lançamento de produção excluído pelo gestor");
    try {
  ...
  async function salvarEdicaoLance(id) {
    ...
    try { await ajustarEstoqueSkuPorProducao(rec, Object.assign({}, rec, patch), "Ajuste automático após edição de apontamento"); } catch (e) { toast(errMsg(e)); }
    await audit("EDITADO", "Lançamento", id, rec, Object.assign({}, rec, patch), "Lançamento de produção corrigido pelo gestor");
    try {
      await recalcularCadeia();
    } catch (e) { toast(errMsg(e)); }
    state.rel.editingId = "";
  ...
  /* ========== LEITURA DE PEDIDO POR FOTO/PDF (Etapa 16) ==========
     Uma leitura completa (cliente + SKU válido + quantidade) passa a
     gerar o Pedido e sua OP imediatamente. Leituras incompletas continuam
     no modo de correção, sem gravar dados errados. */
  function ocrFingerprint(res, sku, client) {
    const raw = [res && res.rawText, sku && sku.code, client && client.name,
      res && res.guessedQuantity, res && res.guessedOrderNumber].join("|");
    if (window.AutomacaoFinal && window.AutomacaoFinal.simpleHash) return "ocr-" + window.AutomacaoFinal.simpleHash(raw);
    let h = 0; for (let i = 0; i < raw.length; i++) h = ((h << 5) - h + raw.charCodeAt(i)) | 0;
    return "ocr-" + Math.abs(h).toString(36);
  }

  async function registrarEventoPedidoAutomatico(status, data) {
    if (window.AutomacaoFinal && window.AutomacaoFinal.registrarExecucao) {
      return window.AutomacaoFinal.registrarExecucao("OCR_PEDIDO", status, data);
    }
    return S.add("automationEvents", Object.assign({ type: "OCR_PEDIDO", status: status, createdAt: Date.now() }, data || {}));
  }

  async function criarPedidoAutomaticoOCR(res, sku, client) {
    const quantity = Number(res && res.guessedQuantity) || 0;
    if (!sku || !client || quantity <= 0) throw new Error("Leitura incompleta para criação automática.");
    const fingerprint = ocrFingerprint(res, sku, client);
    const existing = orders.find(o => o.ocrFingerprint === fingerprint);
    if (existing) {
      await registrarEventoPedidoAutomatico("DUPLICADO", { fingerprint, orderId: existing.id, rawText: res.rawText || "" });
      return { order: existing, op: ops.find(o => o.orderId === existing.id) || null, duplicate: true };
    }

    const correlationId = window.AutomacaoFinal && window.AutomacaoFinal.correlationId
      ? window.AutomacaoFinal.correlationId("OCR") : "OCR-" + Date.now().toString(36);
    const savedOrder = await S.add("orders", {
      number: String(res.guessedOrderNumber || "").trim(),
      client: client.name,
      skuId: sku.id,
      skuCode: sku.code,
      description: "",
      ...
      status: "aguardando",
      source: "ocr-automatica",
      correlationId,
      ocrFingerprint: fingerprint,
      ocrConfidence: res.confidence || "media",
      ocrRawText: res.rawText || "",
      createdAt: Date.now()
    });
    await audit("CRIADO", "Pedido", savedOrder.id, null, savedOrder, "Pedido criado automaticamente pela leitura OCR");

    const opResult = await criarOPAutomaticaParaPedido(savedOrder, {
      source: "ocr-automatica", correlationId,
      note: "OP criada automaticamente pelo pedido lido por OCR"
    });
    const savedOp = opResult.op;
    await registrarEventoPedidoAutomatico("SUCESSO", {
      correlationId, fingerprint, orderId: savedOrder.id, opId: savedOp.id,
      rawText: res.rawText || "", confidence: res.confidence || "media"
    });
    return { order: savedOrder, op: savedOp, duplicate: false };
  }

  async function handlePedidoFile(file) {
    if (!window.LeituraPedido) { toast("Módulo de leitura de pedido não carregado."); return; }
    state.pedidoReading = true;
    renderView();
    try {
      const res = await window.LeituraPedido.lerArquivo(file);
      const sku = (res.guessedCode ? skuByCode(res.guessedCode) : null) || skuByDescription(res.guessedDescription);
      const foundClient = clientByText(res.guessedClient) || guessClientFromText(res.rawText);
      state.newOrder = freshOrderDraft();
      state.newOrder.number = res.guessedOrderNumber || "";
      state.newOrder.skuCode = sku ? sku.code : (res.guessedCode || "");
      if (res.guessedQuantity) state.newOrder.quantity = res.guessedQuantity;
      if (foundClient) state.newOrder.client = foundClient.name;
      state.newOrder._ocrText = res.rawText || "";
      state.newOrder._ocrConfidence = res.confidence || "baixa";

      if (sku && foundClient && Number(res.guessedQuantity) > 0) {
        const created = await criarPedidoAutomaticoOCR(res, sku, foundClient);
        state.newOrder = freshOrderDraft();
        state.newOrderForm = false;
        state.newOrderConfirmMode = false;
        state.pedidoReading = false;
        renderView();
        toast(created.duplicate
          ? "Este pedido OCR já havia sido registrado. Nenhum duplicado foi criado."
          : "Pedido real e OP " + created.op.number + " criados automaticamente.");
        return;
      }
      state.newOrderForm = true;
      /* Etapa 6: sempre abre a telinha de confirmação resumida — mesmo
         quando a leitura não reconheceu tudo, o gestor completa ali
         mesmo (código, cliente ou quantidade) antes de confirmar. */
      state.newOrderConfirmMode = true;
      state.pedidoReading = false;
      renderView();
      toast(sku
        ? "Leitura incompleta: complete os dados para criar o pedido."
        : (res.guessedCode ? `Código "${res.guessedCode}" não encontrado no catálogo — confira.` : "Não consegui reconhecer o código automaticamente — preencha manualmente."));
    } catch (err) {
      state.pedidoReading = false;
      renderView();
      toast((err && err.message) || "Não foi possível ler o arquivo.");
    }
  }
  ...
  function renderPedidos(v) {
    v.innerHTML = `
      <div class="space-y">
        ...
        <div class="card pad">
          <label class="btn btn-outline btn-block photo-upload-label" style="${state.pedidoReading ? "opacity:.6;pointer-events:none" : ""}">
            ${state.pedidoReading ? "⏳ Lendo pedido, aguarde..." : "📷 Ler pedido (foto ou PDF)"}
            <input type="file" id="pedido-file-input" accept="image/*,application/pdf" hidden>
          </label>
          <div class="tiny dim mt8">Tire uma foto do pedido em papel ou envie o PDF recebido do cliente — o sistema tenta preencher o código, a quantidade e o cliente automaticamente.</div>
        </div>
        ...
        ${state.newOrderForm ? (state.newOrderConfirmMode ? pedidoConfirmScreen() : newOrderForm()) : `<button class="btn btn-outline btn-block" data-act="new-order">+ Novo Pedido</button>`}
      </div>`;
  }
  ...
  function orderCard(o) {
    ...
    if (d) {
      const allowed = ORDER_PRODUCTS_BY_DESCRIPTION[d.description] || [];
      const editSkuMatch = d.skuCode ? skuByCode(d.skuCode) : null;
      const editSkuPreview = d.skuCode
        ? (editSkuMatch ? "✓ " + editSkuMatch.description + (editSkuMatch.category ? " · " + editSkuMatch.category : "") : "Código não encontrado no catálogo.")
        : "";
      return `
        <div class="card pad" data-edit-order-id="${o.id}">
          <div class="rowc between mb12"><span class="small" style="font-weight:600">Editando pedido</span>
            <button class="btn-ghost" data-act="cancel-order" data-id="${o.id}">Cancelar</button></div>
          <div class="field"><label>Cliente</label><select data-k="client"><option value="">Selecione...</option>${clientOptions(d.client)}</select></div>
          <div class="field"><label>N.º do pedido</label><input data-k="number" value="${U.esc(d.number)}"></div>
          <div class="field">
            <label>Código do produto (catálogo)</label>
            <input data-k="skuCode" id="eo-sku-code-${o.id}" value="${U.esc(d.skuCode || "")}" placeholder="Ex.: AG C BCP 27">
            <div id="eo-sku-preview-${o.id}" class="tiny dim mt4" style="${editSkuMatch ? "" : "color:var(--danger,#c0392b)"}">${U.esc(editSkuPreview)}</div>
          </div>
          <div class="field"><label>Descrição</label><select data-k="description" id="eo-description-${o.id}"><option value="">Selecione...</option>${ORDER_DESCRIPTIONS.map(x=>`<option value="${U.esc(x)}" ${d.description===x?"selected":""}>${U.esc(x)}</option>`).join("")}</select></div>
          <div class="field-row">
            <div class="field"><label>Produto</label><select data-k="product" id="eo-product-${o.id}"><option value="">Selecione...</option>${allowed.map(x=>`<option value="${U.esc(x)}" ${d.product===x?"selected":""}>${U.esc(x)}</option>`).join("")}</select></div>
            <div class="field"><label>Tipo</label><select data-k="type"><option value="">Selecione...</option>${ORDER_TYPES.map(x=>`<option value="${U.esc(x)}" ${d.type===x?"selected":""}>${U.esc(x)}</option>`).join("")}</select></div>
          </div>
          <div class="field"><label>Quantidade de Folhas</label><select data-k="sheets" id="eo-sheets-${o.id}"><option value="">Selecione...</option>${ORDER_SHEET_OPTIONS.map(x=>`<option value="${x}" ${String(d.sheets)===String(x)?"selected":""}>${x}</option>`).join("")}</select></div>
          <div class="field-row">
            <div class="field"><label>Modelo</label><select data-k="model"><option value="">Selecione...</option>${ORDER_MODELS.map(x=>`<option value="${U.esc(x)}" ${d.model===x?"selected":""}>${U.esc(x)}</option>`).join("")}</select></div>
            <div class="field"><label>Acabamento</label><select data-k="finish"><option value="">Selecione...</option>${ORDER_FINISHES.map(x=>`<option value="${U.esc(x)}" ${d.finish===x?"selected":""}>${U.esc(x)}</option>`).join("")}</select></div>
          </div>
          <div class="field"><label>Quantidade</label><input type="number" data-k="quantity" min="1" value="${d.quantity}"></div>
          <div class="rowc" style="gap:8px"><button class="btn btn-primary" data-act="save-order" data-id="${o.id}">Salvar pedido</button>
          <button class="btn-ghost" data-act="delete-order" data-id="${o.id}">🗑️ Excluir pedido</button></div>
        </div>`;
    }
    const prog = orderProgress(o.id), qty=Number(o.quantity)||0, remaining=Math.max(0,qty-prog.produced), pct=qty>0?Math.min(100,prog.produced/qty*100):0;
    const stO=ORDER_STATUS[o.status]||ORDER_STATUS.aguardando, open=!!state.openOrders[o.id];
    return `
      <div class="card pad"><div class="rowc"><div class="f1">
        <div class="small" style="font-weight:600">${o.number?"Pedido "+U.esc(o.number)+" · ":""}${orderCode(o)} — ${U.esc(o.client)}</div>
        <div class="tiny dim">${o.skuCode?U.esc(o.skuCode)+" · ":""}${o.description?U.esc(o.description)+" · ":""}${U.esc(o.product)}${o.sheets?" · "+U.esc(String(o.sheets))+" folhas":""}${o.type?" · "+U.esc(o.type):""}${o.model?" · "+U.esc(o.model):""}${o.finish?" · "+U.esc(o.finish):""}</div></div>
        <span class="pill ${stO.pill}">${stO.label}</span><button class="btn-ghost" data-act="edit-order" data-id="${o.id}">Editar</button></div>
        <div class="kv mt12"><b>Pedido:</b> ${U.fmt(qty)} un.</div><div class="kv"><b>Produzido:</b> ${U.fmt(prog.produced)} un.</div><div class="kv"><b>Saldo:</b> ${U.fmt(remaining)} un.</div>
        <div class="tick-row mt8"><div class="tick-fill ${pct>=100?"ok":""}" style="width:${pct}%"></div></div>
        <div class="rowc mt12" style="gap:8px;flex-wrap:wrap"><button class="btn-ghost" data-act="toggle-order" data-id="${o.id}">${open?"Ocultar OPs":"Ver OPs e tarefas ("+(opsForOrder(o.id).length+prog.pos.filter(po=>!po.opId).length)+")"}</button>
        ${remaining>0?`<button class="btn-ghost" data-act="new-op-for-order" data-id="${o.id}">➕ Nova OP para este pedido</button>`:""}<button class="btn-ghost" data-act="delete-order" data-id="${o.id}">🗑️ Excluir</button></div>
        ${open?orderOrdersBlock(o):""}</div>`;
  }
  ...
  function renderOPDetalhe(v) {
    ...
    v.innerHTML = `
      ...
      <div class="card pad">
        ...
        <div class="kv"><b>Etapas automáticas:</b> ${Array.isArray(op.stages) && op.stages.length ? U.esc(op.stages.join(" → ")) : U.esc(op.sector || "Produção")}</div>
  ...
  async function addOP() {
    ...
    const baseData = {
      ...
      stages: (selected.length ? selected.slice() : [((state.newOP.sector || ((config && config.sector) || "Produção")).trim() || "Produção")]),
      status: "aguardando",
      startedAt: null,
      finishedAt: null
    };
    ...
    try {
      if (selected.length) {
        ...
        for (const name of selected) {
          const savedOp = await S.add("ops", Object.assign({}, baseData, { component: name, createdAt: Date.now() }));
          await audit("CRIADO", "OP", savedOp.id, null, savedOp, "OP criada automaticamente a partir da ficha técnica");
        }
  ...
  async function setOPStatus(id, status) {
    ...
    try {
      ...
      await audit("ALTERADO", "OP", id, ops.find(x=>x.id===id), Object.assign({}, ops.find(x=>x.id===id)||{}, patch), "Status da OP alterado");
  ...
  /* ========== CONFIRMAR PEDIDO LIDO (Etapa 6) ==========
     Telinha resumida mostrada logo após a Leitura de Pedido por foto/PDF:
     Cliente + Produto (código do catálogo) + Quantidade, com os botões
     CONFIRMAR (salva o pedido e já cria a OP automaticamente) e
     EDITAR (cai no formulário completo de sempre, sem perder nada do
     que já foi reconhecido). Nunca salva nada sozinha — só o gestor
     decide, com um toque. */
  function pedidoConfirmScreen() {
    const no = state.newOrder;
    const sku = no.skuCode ? skuByCode(no.skuCode) : null;
    const skuPreviewText = no.skuCode
      ? (sku ? "✓ " + sku.description + (sku.category ? " · " + sku.category : "") : "Código não encontrado no catálogo — corrija o código ou toque em \"Editar pedido completo\".")
      : "Não reconhecemos o código do produto — informe manualmente.";
    return `
      <div class="card pad">
        <div class="rowc between mb12">
          <span class="small" style="font-weight:600">Confirmar pedido</span>
          <button class="btn-ghost" data-act="cancel-new-order">Cancelar</button>
        </div>
        <div class="tiny dim mb12" style="padding:8px;border:1px solid var(--border);border-radius:8px">
          📷 Dados extraídos automaticamente do arquivo enviado — confira antes de confirmar.
          ${no._ocrText ? `<details class="mt4"><summary style="cursor:pointer">Ver texto reconhecido</summary><pre style="white-space:pre-wrap;font-size:11px;max-height:160px;overflow:auto">${U.esc(no._ocrText)}</pre></details>` : ""}
        </div>
        <div class="field"><label>Cliente</label><select id="pc-client"><option value="">Selecione...</option>${clientOptions(no.client)}</select></div>
        <div class="field">
          <label>Produto (código do catálogo)</label>
          <input id="pc-sku-code" value="${U.esc(no.skuCode)}" placeholder="Ex.: AG C BCP 27">
          <div id="pc-sku-preview" class="tiny dim mt4" style="${sku ? "" : "color:var(--danger,#c0392b)"}">${U.esc(skuPreviewText)}</div>
        </div>
        <div class="field"><label>Quantidade</label><input type="number" id="pc-quantity" min="1" value="${U.esc(no.quantity)}"></div>
        <div class="rowc" style="gap:8px">
          <button class="btn btn-primary f1" data-act="confirm-pedido">✓ Confirmar</button>
          <button class="btn btn-outline f1" data-act="edit-pedido-full">Editar pedido completo</button>
        </div>
        <div class="tiny dim mt8">Ao confirmar, o pedido é salvo e uma Ordem de Produção é criada automaticamente.</div>
      </div>`;
  }

  function newOrderForm() {
    const no = state.newOrder;
    const allowedProducts = ORDER_PRODUCTS_BY_DESCRIPTION[no.description] || [];
    const productOptions = allowedProducts.map(x => `<option value="${U.esc(x)}" ${no.product === x ? "selected" : ""}>${U.esc(x)}</option>`).join("");
    const typeOptions = ORDER_TYPES.map(x => `<option value="${U.esc(x)}" ${no.type === x ? "selected" : ""}>${U.esc(x)}</option>`).join("");
    const modelOptions = ORDER_MODELS.map(x => `<option value="${U.esc(x)}" ${no.model === x ? "selected" : ""}>${U.esc(x)}</option>`).join("");
    const finishOptions = ORDER_FINISHES.map(x => `<option value="${U.esc(x)}" ${no.finish === x ? "selected" : ""}>${U.esc(x)}</option>`).join("");
    const sheetOptions = ORDER_SHEET_OPTIONS.map(x => `<option value="${x}" ${String(no.sheets) === String(x) ? "selected" : ""}>${x}</option>`).join("");
    const skuMatch = no.skuCode ? skuByCode(no.skuCode) : null;
    const skuPreviewText = no.skuCode
      ? (skuMatch ? "✓ " + skuMatch.description + (skuMatch.category ? " · " + skuMatch.category : "") : "Código não encontrado no catálogo — cadastre em \"Catálogo de Produtos\" ou preencha os campos abaixo manualmente.")
      : "";
    return `
      <div class="card pad">
        <div class="rowc between mb12">
        ...
        ${no._ocrText ? `
        <div class="tiny dim mb12" style="padding:8px;border:1px solid var(--border);border-radius:8px">
          📷 Dados extraídos automaticamente do arquivo enviado — confira e corrija se necessário antes de salvar.
          <details class="mt4"><summary style="cursor:pointer">Ver texto reconhecido</summary><pre style="white-space:pre-wrap;font-size:11px;max-height:160px;overflow:auto">${U.esc(no._ocrText)}</pre></details>
        </div>` : ""}
        <div class="field"><label>Cliente</label><select id="no-client"><option value="">Selecione...</option>${clientOptions(no.client)}</select></div>
        ...
        <div class="field">
          <label>Código do produto (catálogo) <span class="tiny dim">— localiza o produto automaticamente</span></label>
          <input id="no-sku-code" value="${U.esc(no.skuCode)}" placeholder="Ex.: AG C BCP 27">
          <div id="no-sku-preview" class="tiny dim mt4" style="${skuMatch ? "" : "color:var(--danger,#c0392b)"}">${U.esc(skuPreviewText)}</div>
        </div>
        <div class="tiny dim mt4 mb4">Os campos abaixo são opcionais quando o código do catálogo é informado.</div>
        <div class="field"><label>Descrição</label>
          <select id="no-description">
            <option value="">Selecione...</option>
            ${ORDER_DESCRIPTIONS.map(x => `<option value="${U.esc(x)}" ${no.description === x ? "selected" : ""}>${U.esc(x)}</option>`).join("")}
          </select>
        </div>
        <div class="field-row">
          <div class="field"><label>Produto</label>
            <select id="no-product" ${no.description ? "" : "disabled"}>
              <option value="">${no.description ? "Selecione..." : "Selecione a descrição primeiro..."}</option>
              ${productOptions}
            </select>
          </div>
        ...
        <div class="field"><label>Quantidade de Folhas</label>
          <select id="no-sheets">
            <option value="">Selecione...</option>
            ${sheetOptions}
          </select>
        ...
        <div class="field"><label>Quantidade do pedido</label><input type="number" id="no-quantity" min="0" required value="${U.esc(no.quantity)}" placeholder="Ex.: 50000"></div>
  ...
  async function doAddTask() {
    ...
    try {
      ...
      await audit("CRIADO", "Tarefa", saved.id, null, saved, "Tarefa adicionada à fila de produção");
      if (isUrgent) {
  ...
  async function finishProductionOrder(id) {
    const before = productionOrders.find(x=>x.id===id);
    try {
      const patch = { status: "finalizada", finishedAt: Date.now() };
      await S.update("productionOrders", id, patch);
      await audit("ALTERADO", "Tarefa", id, before, Object.assign({}, before, patch), "Tarefa marcada como finalizada");
  ...
  /* ========== CATÁLOGO DE PRODUTOS POR CÓDIGO (Etapa 3) ========== */
  /* Entidade nova e independente do cadastro "Produtos" (modelo de
     furação/máquina, seção acima). Aqui o código é a chave usada para
     localizar o item — normalizado (maiúsculas, sem espaços duplicados)
     para casar de forma tolerante com leitura de pedido/voz (Etapas
     futuras) sem travar por causa de maiúsculas/minúsculas ou espaços. */
  /* A mesma chave tolerante é usada por OCR, voz e cadastro: diferenças
     de caixa, acentos, hífens e pontuação não impedem o casamento. */
  function normCode(s) {
    return String(s || "").trim().toUpperCase().normalize("NFD")
  ...
  function skuByCode(code) {
    ...
    if (!n) return null;
    return skus.find(s => normCode(s.code) === n) || null;
  }

  function skuByDescription(description) {
    const n = normText(description);
    if (!n) return null;
    return skus.find(s => {
      const d = normText(s.description);
      return d && (d === n || d.indexOf(n) >= 0 || n.indexOf(d) >= 0);
    }) || null;
  }
  ...
  function renderCatalogo(v) {
    v.innerHTML = `
      <div class="space-y">
        ${skus.length ? skus.slice().sort((a, b) => normCode(a.code).localeCompare(normCode(b.code))).map(skuCard).join("") : '<div class="empty">Nenhum produto cadastrado no catálogo.</div>'}
        ${state.newSkuForm ? newSkuForm() : `<button class="btn btn-outline btn-block" data-act="new-sku">+ Novo produto (catálogo)</button>`}
      </div>`;
  }

  function skuCard(s) {
    const d = state.draftSku[s.id];
    if (d) {
      return `
        <div class="card pad" data-edit-sku-id="${s.id}">
          <div class="rowc between mb12">
            <span class="small" style="font-weight:600">Editando produto</span>
            <button class="btn-ghost" data-act="cancel-sku" data-id="${s.id}">Cancelar</button>
          </div>
          <div class="field"><label>Código do produto</label><input data-k="code" value="${U.esc(d.code)}" placeholder="Ex.: AG C BCP 27"></div>
          <div class="field"><label>Descrição</label><input data-k="description" value="${U.esc(d.description)}" placeholder="Ex.: AGENDA CLÁSSICA BASIC PRETO (13,5 X 20CM)"></div>
          <div class="field-row">
            <div class="field"><label>Categoria</label><input data-k="category" value="${U.esc(d.category)}" placeholder="Ex.: Agenda"></div>
            <div class="field"><label>Unidade</label><input data-k="unit" value="${U.esc(d.unit)}" placeholder="Ex.: UN"></div>
          </div>
          <div class="field"><label>Estoque atual</label><input type="number" data-k="stock" value="${d.stock}"></div>
          <button class="btn btn-primary btn-block mt12" data-act="save-sku" data-id="${s.id}">Salvar produto</button>
        </div>`;
    }
    return `
      <div class="card pad">
        <div class="rowc">
          <div class="f1">
            <div class="small" style="font-weight:600">${U.esc(s.code)}</div>
            <div class="tiny dim">${U.esc(s.description)}${s.category ? " · " + U.esc(s.category) : ""}</div>
          </div>
          <button class="btn-ghost" data-act="edit-sku" data-id="${s.id}">Editar</button>
          <button class="btn-ghost" data-act="delete-sku" data-id="${s.id}">🗑️</button>
        </div>
        <div class="kv mt12"><b>Estoque atual:</b> ${U.fmt(Number(s.stock) || 0)} ${U.esc(s.unit || "UN")}</div>
      </div>`;
  }

  function newSkuForm() {
    const n = state.newSku;
    return `
      <div class="card pad">
        <div class="rowc between mb12">
          <span class="small" style="font-weight:600">Novo produto (catálogo)</span>
          <button class="btn-ghost" data-act="cancel-new-sku">Cancelar</button>
        </div>
        <div class="field"><label>Código do produto</label><input id="ns-code" value="${U.esc(n.code)}" placeholder="Ex.: AG C BCP 27"></div>
        <div class="field"><label>Descrição</label><input id="ns-description" value="${U.esc(n.description)}" placeholder="Ex.: AGENDA CLÁSSICA BASIC PRETO (13,5 X 20CM)"></div>
        <div class="field-row">
          <div class="field"><label>Categoria</label><input id="ns-category" value="${U.esc(n.category)}" placeholder="Ex.: Agenda"></div>
          <div class="field"><label>Unidade</label><input id="ns-unit" value="${U.esc(n.unit)}" placeholder="Ex.: UN"></div>
        </div>
        <div class="field"><label>Estoque atual</label><input type="number" id="ns-stock" value="${n.stock}"></div>
        <button class="btn btn-primary btn-block mt12" data-act="save-new-sku">Adicionar produto</button>
      </div>`;
  }
  ...
  async function addSku() {
    ...
    if (!code || !description) { toast("Informe pelo menos o código e a descrição do produto."); return; }
    if (skuByCode(code)) { toast("Já existe um produto cadastrado com esse código."); return; }
    try {
      const saved = await S.add("skus", {
        code, description,
        category: (n.category || "").trim(),
        unit: (n.unit || "").trim() || "UN",
        stock: Number(n.stock) || 0
      });
      await audit("CRIADO", "Catálogo", saved.id, null, saved, "Produto adicionado ao catálogo");
      state.newSku = { code: "", description: "", category: "", unit: "UN", stock: 0 };
      state.newSkuForm = false;
  ...
  async function saveSku(id) {
    const d = state.draftSku[id];
    if (!d) return;
    const code = (d.code || "").trim();
    const description = (d.description || "").trim();
    if (!code || !description) { toast("Informe pelo menos o código e a descrição do produto."); return; }
    const dup = skus.find(s => s.id !== id && normCode(s.code) === normCode(code));
    if (dup) { toast("Já existe um produto cadastrado com esse código."); return; }
    try {
      const before = skus.find(s => s.id === id) || null;
      const patch = {
        code, description,
      ...
      await S.update("skus", id, patch);
      await audit("EDITADO", "Catálogo", id, before, Object.assign({}, before || {}, patch), "Produto do catálogo alterado");
      delete state.draftSku[id];
      toast("Produto salvo");
      renderView();
    } catch (e) { toast(errMsg(e)); }
  }
  ...
  async function deleteSku(id) {
    ...
    try {
      const before = skus.find(s => s.id === id) || null;
      await S.remove("skus", id);
      await audit("EXCLUÍDO", "Catálogo", id, before, null, "Produto excluído do catálogo");
  ...
  /* ========== CLIENTES (Etapa 2) ========== */
  /* Cadastro simples de clientes. Usado para selecionar o cliente no
     Pedido (em vez de digitar texto livre). Pedidos antigos que tenham
     um nome de cliente fora do cadastro continuam preservados — o nome
     atual aparece como opção extra até o gestor trocar por um cadastrado.

     Etapa 2 (revisão): cada cliente pode ter "apelidos/variações" — outras
     formas como o nome pode aparecer num pedido em papel/PDF (ex.: cliente
     "Papelaria" com apelido "Papelaria Central"). Isso prepara o cadastro
     para as próximas etapas (leitura por foto/PDF e comando por voz), que
     vão localizar o cliente pelo texto reconhecido em vez de um campo
     digitado à mão. O nome cadastrado continua sendo o único usado para
     gravar no Pedido — apelido é só um auxílio de busca. */
  ...
  function clientAliasList(c) {
    return String((c && c.aliases) || "").split(",").map(a => a.trim()).filter(Boolean);
  }

  /* Localiza um cliente cadastrado pelo nome OU por um de seus apelidos,
     de forma tolerante a maiúsculas/acentos. Usado hoje pelo cadastro; as
     próximas etapas (OCR/voz) vão reaproveitar esta mesma função. */
  function clientByText(text) {
    const n = normText(text);
    if (!n) return null;
    return clients.find(c => normText(c.name) === n || clientAliasList(c).some(a => normText(a) === n)) || null;
  }

  function clientOptions(selectedName) {
    const names = clients.map(c => c.name);
    const extra = selectedName && !names.includes(selectedName) ? [selectedName] : [];
    return names.concat(extra).map(n => `<option value="${U.esc(n)}" ${n === selectedName ? "selected" : ""}>${U.esc(n)}</option>`).join("");
  }

  /* Tenta achar, em qualquer lugar de um texto (ex.: reconhecido por OCR),
     um cliente cadastrado — pelo nome ou por um apelido — mesmo que
     apareça no meio de outras palavras. Usado pela Leitura de Pedido por
     foto/PDF (Etapa 5); não decide sozinho, só sugere no formulário. */
  function guessClientFromText(text) {
    const n = normText(text);
    if (!n) return null;
    return clients.find(c => (normText(c.name) && n.indexOf(normText(c.name)) >= 0) ||
      clientAliasList(c).some(a => normText(a) && n.indexOf(normText(a)) >= 0)) || null;
  }

  function renderClientes(v) {
    v.innerHTML = `
      <div class="space-y">
        ${clients.length ? clients.slice().sort((a, b) => (a.name || "").localeCompare(b.name || "")).map(clientCard).join("") : '<div class="empty">Nenhum cliente cadastrado.</div>'}
        ${state.newClientForm ? newClientForm() : `<button class="btn btn-outline btn-block" data-act="new-client">+ Novo cliente</button>`}
      </div>`;
  }

  function clientCard(c) {
    const d = state.draftClient[c.id];
    if (d) {
      return `
        <div class="card pad" data-edit-client-id="${c.id}">
          <div class="rowc between mb12">
            <span class="small" style="font-weight:600">Editando cliente</span>
            <button class="btn-ghost" data-act="cancel-client" data-id="${c.id}">Cancelar</button>
          </div>
          <div class="field"><label>Nome do cliente</label><input data-k="name" value="${U.esc(d.name)}"></div>
          <div class="field"><label>Apelidos / variações <span class="tiny dim">(opcional, separados por vírgula)</span></label><input data-k="aliases" value="${U.esc(d.aliases || "")}" placeholder="Ex.: Papelaria Central, Papelaria SP"></div>
          <button class="btn btn-primary btn-block mt12" data-act="save-client" data-id="${c.id}">Salvar cliente</button>
        </div>`;
    }
    const usedInOrders = orders.filter(o => o.client === c.name).length;
    const aliases = clientAliasList(c);
    return `
      <div class="card pad">
        <div class="rowc">
          <div class="f1">
            <div class="small" style="font-weight:600">${U.esc(c.name)}</div>
            <div class="tiny dim">${usedInOrders ? usedInOrders + " pedido(s)" : "Sem pedidos"}${aliases.length ? " · apelidos: " + U.esc(aliases.join(", ")) : ""}</div>
          </div>
          <button class="btn-ghost" data-act="edit-client" data-id="${c.id}">Editar</button>
          <button class="btn-ghost" data-act="delete-client" data-id="${c.id}">🗑️</button>
        </div>
      </div>`;
  }
  ...
  function newClientForm() {
    return `
      <div class="card pad">
        <div class="rowc between mb12">
          <span class="small" style="font-weight:600">Novo cliente</span>
          <button class="btn-ghost" data-act="cancel-new-client">Cancelar</button>
        </div>
        <div class="field"><label>Nome do cliente</label><input id="nc-name" value="${U.esc(state.newClient.name)}" placeholder="Ex.: Papelaria"></div>
        <div class="field"><label>Apelidos / variações <span class="tiny dim">(opcional, separados por vírgula)</span></label><input id="nc-aliases" value="${U.esc(state.newClient.aliases || "")}" placeholder="Ex.: Papelaria Central, Papelaria SP"></div>
        <button class="btn btn-primary btn-block mt12" data-act="save-new-client">Adicionar cliente</button>
      </div>`;
  }

  async function addClient() {
    const name = (state.newClient.name || "").trim();
    const aliases = (state.newClient.aliases || "").trim();
    if (!name) { toast("Informe o nome do cliente."); return; }
    if (clients.some(c => c.name.trim().toLowerCase() === name.toLowerCase())) { toast("Já existe um cliente com esse nome."); return; }
    try {
      const saved = await S.add("clients", { name, aliases });
      await audit("CRIADO", "Cliente", saved.id, null, saved, "Cliente cadastrado");
      state.newClient = { name: "", aliases: "" };
      state.newClientForm = false;
  ...
  async function saveClient(id) {
    const d = state.draftClient[id];
    if (!d) return;
    const name = (d.name || "").trim();
    const aliases = (d.aliases || "").trim();
    if (!name) { toast("Informe o nome do cliente."); return; }
    if (clients.some(c => c.id !== id && c.name.trim().toLowerCase() === name.toLowerCase())) { toast("Já existe um cliente com esse nome."); return; }
    try {
      const before = clients.find(c => c.id === id) || null;
      const patch = { name, aliases };
      await S.update("clients", id, patch);
      await audit("EDITADO", "Cliente", id, before, Object.assign({}, before || {}, patch), "Cliente alterado");
      delete state.draftClient[id];
      toast("Cliente salvo");
      renderView();
    } catch (e) { toast(errMsg(e)); }
  }
  ...
  async function deleteClient(id) {
    ...
    const msg = usedInOrders
      ? `Este cliente é usado em ${usedInOrders} pedido(s). Os pedidos existentes NÃO serão alterados, só o cadastro será removido. Excluir mesmo assim?`
      : "Excluir este cliente?";
    if (!confirm(msg)) return;
    try {
      await S.remove("clients", id);
      await audit("EXCLUÍDO", "Cliente", id, c, null, "Cliente excluído");
    ...
    if (d) {
      return `
        <div class="card pad" data-edit-emp-id="${em.id}">
          ...
          <div class="field"><label>Nível de acesso</label><select data-k="accessLevel"><option value="operador" ${d.accessLevel === "operador" ? "selected" : ""}>Operador</option><option value="supervisor" ${d.accessLevel === "supervisor" ? "selected" : ""}>Supervisor</option></select></div>
          <div class="field"><label>Status</label>
            <select data-k="status">
    ...
    return `
      <div class="card pad">
        ...
        <div class="field"><label>Função</label><input id="ne-role" value="${U.esc(ne.role)}" placeholder="Ex.: Operador de máquina"></div>
          <div class="field"><label>Nível de acesso</label><select id="ne-accessLevel"><option value="operador" ${ne.accessLevel === "operador" ? "selected" : ""}>Operador</option><option value="supervisor" ${ne.accessLevel === "supervisor" ? "selected" : ""}>Supervisor</option></select></div>
        <div class="field-row">
  ...
  function bindAllExtra() {
    document.querySelectorAll("[data-edit-mach-id]").forEach(c => {
      ...
      const ed = c.querySelector('[data-k="description"]');
      if (ed) ed.addEventListener("change", () => { d.description=ed.value; d.product=""; renderView(); });
    });
    ...
    document.querySelectorAll("[data-edit-sku-id]").forEach(c => {
      const id = c.getAttribute("data-edit-sku-id");
      const d = state.draftSku[id];
      if (!d) return;
      c.querySelectorAll("[data-k]").forEach(el => {
        const k = el.getAttribute("data-k");
        el.addEventListener("input", () => { d[k] = el.value; });
      });
    });
    ["ns-code", "ns-description", "ns-category", "ns-unit", "ns-stock"].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      const k = id.replace("ns-", "");
      el.addEventListener("input", () => { state.newSku[k] = el.value; });
    });

    document.querySelectorAll("[data-edit-client-id]").forEach(c => {
      const id = c.getAttribute("data-edit-client-id");
      const d = state.draftClient[id];
      if (!d) return;
      c.querySelectorAll("[data-k]").forEach(el => {
        const k = el.getAttribute("data-k");
    ...
    if (ncName) ncName.addEventListener("input", () => { state.newClient.name = ncName.value; });
    const ncAliases = document.getElementById("nc-aliases");
    if (ncAliases) ncAliases.addEventListener("input", () => { state.newClient.aliases = ncAliases.value; });
    ...
    ["ne-name", "ne-role", "ne-pin", "ne-pin2", "ne-status", "ne-accessLevel"].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
    ...
    document.querySelectorAll("[data-edit-order-id]").forEach(c => {
      ...
      c.querySelectorAll("[data-k]").forEach(el => {
        ...
        el.addEventListener(ev, () => {
          d[k] = el.value;
          /* CORREÇÃO: ao trocar a Descrição na edição do pedido, a lista
             de Produtos precisa ser recarregada de acordo com a nova
             descrição (o Produto renderizado antes é fixo, calculado só
             na hora do render). Se o produto já selecionado não existir
             na nova descrição, ele é limpo — senão continua o mesmo. */
          if (k === "description") {
            const allowedNow = ORDER_PRODUCTS_BY_DESCRIPTION[d.description] || [];
            if (!allowedNow.includes(d.product)) d.product = "";
            renderView();
          }
          if (k === "skuCode") {
            const preview = document.getElementById("eo-sku-preview-" + id);
            if (preview) {
              const v = el.value.trim();
              const sku = v ? skuByCode(v) : null;
              preview.textContent = !v ? "" : (sku ? "✓ " + sku.description + (sku.category ? " · " + sku.category : "") : "Código não encontrado no catálogo.");
              preview.style.color = (v && !sku) ? "var(--danger,#c0392b)" : "";
            }
          }
        });
      });
    });
    ["no-client", "no-number", "no-product", "no-sheets", "no-model", "no-quantity", "no-type", "no-finish"].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
    ...
    const noSkuCode = document.getElementById("no-sku-code");
    if (noSkuCode) noSkuCode.addEventListener("input", () => {
      state.newOrder.skuCode = noSkuCode.value;
      const preview = document.getElementById("no-sku-preview");
      if (!preview) return;
      const v = noSkuCode.value.trim();
      const sku = v ? skuByCode(v) : null;
      preview.textContent = !v ? "" : (sku ? "✓ " + sku.description + (sku.category ? " · " + sku.category : "") : "Código não encontrado no catálogo — cadastre em \"Catálogo de Produtos\" ou preencha os campos abaixo manualmente.");
      preview.style.color = (v && !sku) ? "var(--danger,#c0392b)" : "";
    });
    /* ---- Confirmar Pedido lido (Etapa 6) ---- */
    const pcClient = document.getElementById("pc-client");
    if (pcClient) pcClient.addEventListener("change", () => { state.newOrder.client = pcClient.value; });
    const pcQuantity = document.getElementById("pc-quantity");
    if (pcQuantity) pcQuantity.addEventListener("input", () => { state.newOrder.quantity = pcQuantity.value; });
    const pcSkuCode = document.getElementById("pc-sku-code");
    if (pcSkuCode) pcSkuCode.addEventListener("input", () => {
      state.newOrder.skuCode = pcSkuCode.value;
      const preview = document.getElementById("pc-sku-preview");
      if (!preview) return;
      const v = pcSkuCode.value.trim();
      const sku = v ? skuByCode(v) : null;
    ...
    if (noDescription) noDescription.addEventListener("change", () => {
      state.newOrder.description = noDescription.value;
      /* Produto só é limpo se não existir na nova descrição — evita
         perder uma seleção que continua válida. */
      const allowedNow = ORDER_PRODUCTS_BY_DESCRIPTION[state.newOrder.description] || [];
      if (!allowedNow.includes(state.newOrder.product)) state.newOrder.product = "";
      renderView();
    });
  ...
  function canManage() { return state.authed === true; }
  async function audit(action, entity, entityId, before, after, note) {
    try {
      await S.add("auditLogs", {
        action, entity, entityId: entityId || "",
        before: before ? JSON.parse(JSON.stringify(before)) : null,
        after: after ? JSON.parse(JSON.stringify(after)) : null,
        note: note || "", actor: "Gestor", createdAt: Date.now()
      });
    } catch (e) { console.warn("Falha ao registrar histórico", e); }
  }

  function renderHistorico(v) {
    const rows = auditLogs.slice().sort((a,b)=>(b.createdAt||0)-(a.createdAt||0));
    v.innerHTML = `
      <div class="space-y">
        <div class="card pad"><div class="small" style="font-weight:700">Histórico de alterações</div>
        <div class="tiny dim mt8">Trilha completa de pedidos, OPs, tarefas, apontamentos, estoque e automações.</div></div>
        ${rows.length ? rows.slice(0,300).map(r => `
          <div class="card pad">
            <div class="rowc between"><b>${U.esc(r.action || "ALTERAÇÃO")}</b><span class="tiny dim">${r.createdAt ? new Date(r.createdAt).toLocaleString("pt-BR") : "—"}</span></div>
            <div class="kv mt8">${U.esc(r.entity || "—")} · ${U.esc(r.entityId || "—")}</div>
            ${r.note ? `<div class="tiny dim mt4">${U.esc(r.note)}</div>` : ""}
            <div class="tiny dim mt8">Responsável: ${U.esc(r.actor || "Sistema")} · Origem: ${U.esc(r.source || "manual")}${r.correlationId ? " · Correlação: " + U.esc(r.correlationId) : ""}</div>
            ${(r.before || r.after) ? `<details class="mt8"><summary class="tiny dim" style="cursor:pointer">Ver alteração registrada</summary><pre style="white-space:pre-wrap;word-break:break-word;font-size:11px;max-height:180px;overflow:auto;margin:8px 0 0">${U.esc(JSON.stringify({ antes: r.before || null, depois: r.after || null }, null, 2))}</pre></details>` : ""}
          </div>`).join("") : '<div class="empty">Nenhuma alteração registrada ainda.</div>'}
      </div>`;
  }
  ...
  async function addOrder() {
    ...
    const client = String(n.client || "").trim();
    const skuCode = String(n.skuCode || "").trim();
    const sku = skuCode ? skuByCode(skuCode) : null;
    const description = String(n.description || "").trim();
    const product = String(n.product || "").trim();
    const type = String(n.type || "").trim();
    const model = String(n.model || "").trim();
    const finish = String(n.finish || "").trim();
    const quantity = Number(n.quantity) || 0;
    const sheets = Number(n.sheets) || 0;

    if (!client) { toast("Selecione o cliente."); return; }
    if (quantity <= 0) { toast("Informe uma quantidade maior que zero."); return; }
    if (skuCode && !sku) { toast("Código não encontrado no catálogo. Cadastre em \"Catálogo de Produtos\" ou deixe o campo em branco."); return; }

    /* Com o código do catálogo, o produto é localizado automaticamente e
       os campos antigos (descrição/tipo/folhas/modelo/acabamento) ficam
       livres — continuam disponíveis para detalhar o pedido, mas não são
       obrigatórios. Sem o código, mantém-se a regra original. */
    if (!sku) {
      const allowedProducts = ORDER_PRODUCTS_BY_DESCRIPTION[description] || [];
      if (!description || !product || !sheets || !type || !model || !finish) {
        toast("Preencha descrição, produto, quantidade de folhas, tipo, modelo e acabamento — ou informe o código do produto no catálogo.");
        return;
      }
      if (!allowedProducts.includes(product)) {
        toast("Produto inválido para a descrição selecionada.");
        return;
      }
      if (!ORDER_SHEET_OPTIONS.includes(sheets)) {
        toast("Quantidade de folhas inválida.");
        return;
      }
    }
    try {
      const saved = await S.add("orders", {
        number: String(n.number || "").trim(),
        client,
        skuId: sku ? sku.id : "",
        skuCode: sku ? sku.code : "",
        description,
        product: sku ? sku.description : product,
        sheets,
        type,
        model,
        finish,
      ...
      await audit("CRIADO", "Pedido", saved.id, null, saved, "Novo pedido cadastrado");
      const opResult = await criarOPAutomaticaParaPedido(saved, {
        source: "pedido-automatico", note: "OP criada automaticamente após cadastro do pedido"
      });
      state.newOrder = freshOrderDraft();
      state.newOrderForm = false;
      toast("Pedido cadastrado · OP " + opResult.op.number + " criada automaticamente");
      renderView();
    } catch (e) { toast(errMsg(e)); }
  }

  /* Etapa 6: número automático para a OP criada a partir de um pedido
     confirmado na leitura por foto/PDF (o gestor não digita nada aqui).
     Simplesmente pega o maior número já usado em qualquer OP e soma 1 —
     nunca colide com números manuais existentes. O fluxo manual de OP
     (addOP, acima) continua pedindo o número digitado pelo gestor,
     como sempre funcionou na fábrica. */
  function nextAutoOPNumber() {
    let max = 0;
    S.get("ops").forEach(o => {
  ...
  async function criarOPAutomaticaParaPedido(order, opts) {
    if (!order || !order.id) throw new Error("Pedido inválido para criar OP.");
    const options = opts || {};
    const existing = S.get("ops").find(o => o.orderId === order.id && o.source === "pedido-automatico");
    if (existing) return { op: existing, duplicate: true };
    const correlationId = options.correlationId || (window.AutomacaoFinal && window.AutomacaoFinal.correlationId
      ? window.AutomacaoFinal.correlationId("PED") : "PED-" + Date.now().toString(36));
    const opNumber = nextAutoOPNumber();
    const setor = (config && config.sector) || "Setor de Furação";
    const savedOp = await S.add("ops", {
      orderId: order.id,
      number: opNumber,
      ...
      quantityProduced: 0,
      remainingQuantity: Number(order.quantity) || 0,
      priority: "media",
      component: "",
      stages: [setor],
      status: "aguardando",
      source: "pedido-automatico",
      correlationId,
      startedAt: null,
      finishedAt: null,
      createdAt: Date.now()
    });
    await audit("CRIADO", "OP", savedOp.id, null, savedOp, options.note || "OP criada automaticamente a partir de pedido");
    if (window.AutomacaoFinal && window.AutomacaoFinal.registrarExecucao) {
      await window.AutomacaoFinal.registrarExecucao("PEDIDO_OP", "SUCESSO", {
        correlationId, orderId: order.id, opId: savedOp.id, source: options.source || "pedido-automatico"
      });
    }
    return { op: savedOp, duplicate: false, correlationId };
  }

  /* Etapa 6: confirma o pedido lido por foto/PDF na telinha resumida
     (pedidoConfirmScreen) — salva o Pedido e, na sequência, cria
     automaticamente a Ordem de Produção correspondente (número gerado
     por nextAutoOPNumber, produto/quantidade vindos do próprio pedido).
     Exige cliente selecionado e um código de catálogo válido — sem
     código reconhecido, o gestor usa "Editar pedido completo" para
     preencher pelo formulário antigo (descrição/produto/tipo/etc.). */
  async function confirmPedidoLido() {
    const n = state.newOrder;
    const client = String(n.client || "").trim();
    const skuCode = String(n.skuCode || "").trim();
    const sku = skuCode ? skuByCode(skuCode) : null;
    ...
    if (!sku) {
      toast(skuCode
        ? 'Código "' + skuCode + '" não encontrado no catálogo — corrija o código ou toque em "Editar pedido completo".'
        : "Informe o código do produto (catálogo).");
      return;
    }
    if (quantity <= 0) { toast("Informe uma quantidade maior que zero."); return; }

    try {
      const created = await criarPedidoAutomaticoOCR({
        rawText: n._ocrText || "", guessedCode: sku.code, guessedClient: client,
        guessedQuantity: quantity, guessedOrderNumber: String(n.number || "").trim(),
        confidence: n._ocrConfidence || "media"
      }, sku, clientByText(client) || { name: client });
      state.newOrder = freshOrderDraft();
      state.newOrderForm = false;
      state.newOrderConfirmMode = false;
      toast(created.duplicate
        ? "Este pedido OCR já havia sido registrado. Nenhum duplicado foi criado."
        : "Pedido confirmado — OP " + created.op.number + " criada automaticamente.");
      renderView();
    } catch (e) { toast(errMsg(e)); }
  }

  async function saveOrder(id) {
    if (!canManage()) { toast("Acesso de gestor necessário."); return; }
    const d=state.draftOrder[id], before=orders.find(x=>x.id===id); if(!d||!before)return;
    const skuCode=String(d.skuCode||"").trim();
    const sku=skuCode?skuByCode(skuCode):null;
    if (skuCode && !sku) { toast("Código não encontrado no catálogo. Cadastre em \"Catálogo de Produtos\" ou deixe o campo em branco."); return; }
    const description=String(d.description||"").trim(), product=String(d.product||"").trim();
    const sheets=Number(d.sheets)||0;
    const patch={
      client:String(d.client||"").trim(),number:String(d.number||"").trim(),
      skuId: sku ? sku.id : "", skuCode: sku ? sku.code : "",
      description,product: sku ? sku.description : product,sheets,
      type:String(d.type||"").trim(),model:String(d.model||"").trim(),finish:String(d.finish||"").trim(),quantity:Number(d.quantity)||0
    };
    if (!patch.client || patch.quantity<=0) { toast("Revise os campos do pedido."); return; }
    if (!sku) {
      const allowed=ORDER_PRODUCTS_BY_DESCRIPTION[description]||[];
      if(!description||!allowed.includes(product)||!sheets||!ORDER_SHEET_OPTIONS.includes(sheets)||!ORDER_TYPES.includes(patch.type)||!ORDER_MODELS.includes(patch.model)||!ORDER_FINISHES.includes(patch.finish)){toast("Revise os campos do pedido. Produto deve ser compatível com a descrição e a quantidade de folhas deve ser selecionada — ou informe o código do produto no catálogo.");return;}
    }
    try{await S.update("orders",id,patch); await audit("EDITADO","Pedido",id,before,Object.assign({},before,patch),"Pedido alterado pelo gestor"); delete state.draftOrder[id]; toast("Pedido salvo"); renderView();}catch(e){toast(errMsg(e));}
  }

  async function deleteOrder(id) {
    if (!canManage()) { toast("Acesso de gestor necessário."); return; }
    const o=orders.find(x=>x.id===id); if(!o)return;
    if(!confirm("Excluir este pedido? As OPs, tarefas e lançamentos vinculados também serão excluídos. O histórico da exclusão será mantido."))return;
    try{
      const relatedPO=productionOrders.filter(x=>x.orderId===id), relatedProd=productions.filter(x=>x.orderId===id), relatedOps=ops.filter(x=>x.orderId===id);
      for(const r of relatedProd) await S.remove("productions",r.id);
      for(const r of relatedPO) await S.remove("productionOrders",r.id);
      for(const r of relatedOps) await S.remove("ops",r.id);
      await S.remove("orders",id);
      await audit("EXCLUÍDO","Pedido",id,o,null,"Pedido e registros vinculados excluídos pelo gestor");
      toast("Pedido excluído"); renderView();
    }catch(e){toast(errMsg(e));}
  }
  ...
  async function saveMachine(id) {
    ...
    try {
      ...
      await audit("EDITADO", "Máquina", id, orig, Object.assign({}, orig, patch), "Cadastro/status da máquina alterado");
  ...
  async function saveProduct(id) {
    ...
    try {
      ...
      await audit("EDITADO", "Produto", id, orig, Object.assign({}, orig, patch), "Ficha técnica do produto alterada");
  ...
  async function addMachine() {
    ...
    try {
      const saved = await S.add("machines", {
        name, type: state.newMachine.type,
        status: "parada", currentProduct: "", client: "",
        capacityHour: 0, capacityDay: 0
      });
      await audit("CRIADO", "Máquina", saved.id, null, saved, "Máquina cadastrada");
  ...
  async function addDefectRecord() {
    ...
    try {
      const saved = await S.add("defectRecords", {
      ...
      await audit("CRIADO", "Defeito", saved.id, null, saved, "Registro de qualidade criado");
  ...
  async function addEmployee() {
    ...
    try {
      const saved = await S.add("employees", {
        name,
        role: (state.newEmployee.role || "").trim(),
        accessLevel: state.newEmployee.accessLevel || "operador",
      ...
      await audit("CRIADO", "Funcionário", saved.id, null, Object.assign({}, saved, { pinHash: saved.pinHash ? "[protegido]" : "" }), "Funcionário cadastrado");
      state.newEmployee = { name: "", role: "", pin: "", pin2: "", status: "ativo", accessLevel: "operador", allowedMachines: [], photoData: "" };
  ...
  async function saveEmployee(id) {
    ...
    const patch = {
      name: (d.name || "").trim() || orig.name,
      role: (d.role || "").trim(),
      accessLevel: d.accessLevel || "operador",
    ...
    try {
      ...
      await audit("EDITADO", "Funcionário", id, orig, Object.assign({}, orig, patch), "Permissões/cadastro do funcionário alterados");
  ...
  async function toggleEmployeeStatus(id) {
    ...
    try {
      ...
      await audit("ALTERADO", "Funcionário", id, em, Object.assign({}, em, {status: next}), next === "ativo" ? "Funcionário ativado" : "Funcionário desativado");
  ...
  async function saveConfig() {
    ...
    try {
      ...
      await S.update("config", "default", patch);
      await audit("EDITADO", "Configuração", "default", before, Object.assign({}, before, patch), "Ajustes do sistema alterados");
  ...
  async function changePin() {
    ...
    try {
      ...
      await audit("ALTERADO", "Segurança", "admin-pin", null, { changed: true }, "PIN do gestor atualizado");
      p1.value = ""; p2.value = "";
  ...
  async function setOpPin() {
    ...
    try {
      ...
      await audit("ALTERADO", "Segurança", "operator-pin", null, { changed: true }, "PIN do operador atualizado");
      p1.value = ""; p2.value = "";
      renderView();
  ...
  async function clearOpPin() {
    ...
    try {
      ...
      await audit("ALTERADO", "Segurança", "operator-pin", { enabled: true }, { enabled: false }, "PIN do operador removido");
      renderView();
  ...
  async function onChangeGlobal(e) {
    ...
    if (!file) return;
    if (input.id === "pedido-file-input") { await handlePedidoFile(file); input.value = ""; return; }
    try {
  ...
  function onClickGlobal(e) {
    ...
    if (a === "login") doLogin();
      ...
      const c = clients.find(x => x.id === id);
      if (c) state.draftClient[id] = { name: c.name, aliases: c.aliases || "" };
      renderView();
    }
    else if (a === "cancel-client") { delete state.draftClient[id]; renderView(); }
    else if (a === "save-client") saveClient(id);
    else if (a === "delete-client") deleteClient(id);
    else if (a === "new-client") { state.newClientForm = true; state.newClient = { name: "", aliases: "" }; renderView(); }
    else if (a === "cancel-new-client") { state.newClientForm = false; renderView(); }
    else if (a === "save-new-client") addClient();

    else if (a === "edit-sku") {
      const s = skus.find(x => x.id === id);
      if (s) state.draftSku[id] = { code: s.code, description: s.description, category: s.category || "", unit: s.unit || "UN", stock: s.stock || 0 };
      renderView();
    }
    else if (a === "cancel-sku") { delete state.draftSku[id]; renderView(); }
    else if (a === "save-sku") saveSku(id);
    else if (a === "delete-sku") deleteSku(id);
    else if (a === "new-sku") { state.newSkuForm = true; state.newSku = { code: "", description: "", category: "", unit: "UN", stock: 0 }; renderView(); }
    else if (a === "cancel-new-sku") { state.newSkuForm = false; renderView(); }
    else if (a === "save-new-sku") addSku();
    ...
    else if (a === "new-employee") { state.newEmployeeForm = true; state.newEmployee = { name: "", role: "", pin: "", pin2: "", status: "ativo", accessLevel: "operador", allowedMachines: [], photoData: "" }; renderView(); }
      ...
      if (em) state.draftEmployee[id] = {
        ...
        status: em.status || "ativo", accessLevel: em.accessLevel || "operador", allowedMachines: (em.allowedMachines || []).slice(),
    ...
    else if (a === "cancel-new-task") { state.newTaskForm = false; state.newTask = freshTaskDraft(); renderView(); }
      ...
      if (o) state.draftOrder[id] = {
        client: o.client, number: o.number || "", skuCode: o.skuCode || "", description: o.description || "", product: o.product || "",
        sheets: o.sheets || "", type: o.type || "", model: o.model || "", finish: o.finish || "", quantity: o.quantity, status: o.status || "aguardando"
      };
      renderView();
    }
    else if (a === "cancel-order") { delete state.draftOrder[id]; renderView(); }
    else if (a === "save-order") saveOrder(id);
    else if (a === "delete-order") deleteOrder(id);
    else if (a === "new-order") { state.newOrderForm = true; state.newOrderConfirmMode = false; state.newOrder = freshOrderDraft(); renderView(); }
    else if (a === "cancel-new-order") { state.newOrderForm = false; state.newOrderConfirmMode = false; renderView(); }
    else if (a === "save-new-order") addOrder();
    else if (a === "confirm-pedido") confirmPedidoLido();
    else if (a === "edit-pedido-full") { state.newOrderConfirmMode = false; renderView(); }
      ...
      ["cp_config", "cp_machines", "cp_products", "cp_productions", "cp_employees", "cp_orders", "cp_productionOrders", "cp_ops", "cp_materials", "cp_stockMovements", "cp_defectRecords", "cp_auditLogs"].forEach(k => { try { localStorage.removeItem(k); } catch (er) { } });
      location.reload();
    }
  }
