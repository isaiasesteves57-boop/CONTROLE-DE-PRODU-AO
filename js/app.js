...
(function () {
  ...
  function orderCard(o) {
    ...
    return `
      <div class="card pad">
        <div class="rowc">
          <div class="f1">
            <div class="small" style="font-weight:600">${U.esc(o.client)}</div>
        ...
        <div class="kv"><b>Restante:</b> ${U.fmt(remaining)} un.</div>
        <div class="tick-row mt8"><div class="tick-fill ${pct >= 100 ? "ok" : ""}" style="width:${pct}%"></div></div>
        <button class="btn-ghost mt12" data-act="toggle-order" data-id="${o.id}">${open ? "Ocultar ordens de produção" : "Ver ordens de produção (" + prog.pos.length + ")"}</button>
  ...
  function orderOrdersBlock(o) {
    ...
    return `
      <div class="space-y3 mt12">
        ${prog.pos.map(poCard).join("")}
        ${state.newPOForm[o.id] ? newPOForm(o) : `<button class="btn btn-outline btn-block" data-act="new-po" data-id="${o.id}">+ Nova Ordem de Produção</button>`}
      </div>`;
  }

  function poCard(po) {
    const stPO = PO_STATUS[po.status] || PO_STATUS.aguardando;
    const planned = Number(po.quantityPlanned) || 0;
    ...
    return `
      <div class="card pad" style="background:var(--panel)">
        <div class="rowc">
          <div class="f1">
            <div class="small" style="font-weight:600">${U.esc(po.product)}</div>
            <div class="tiny dim">${U.esc(po.machineName)} · ${po.employeeName ? "Funcionário: " + U.esc(po.employeeName) + " · " : ""}${po.operacao === "retrabalho" ? "Retrabalho" : "Produção"}</div>
          </div>
          <span class="pill ${stPO.pill}">${stPO.label}</span>
        </div>
        <div class="kv mt12"><b>Planejado:</b> ${U.fmt(planned)} un.</div>
  ...
  function newPOForm(o) {
    const np = state.newPO[o.id] || { machineId: "", employeeId: "", product: "", quantityPlanned: "", operacao: "producao" };
    return `
      <div class="card pad">
        <div class="rowc between mb12">
          <span class="small" style="font-weight:600">Nova Ordem de Produção</span>
          <button class="btn-ghost" data-act="cancel-new-po" data-id="${o.id}">Cancelar</button>
        </div>
        <div class="field"><label>Funcionário responsável <span class="tiny dim">(obrigatório)</span></label>
          <select data-npo-k="employeeId" data-order-id="${o.id}">
            <option value="">Selecione...</option>
            ${employees.filter(e => e.status === "ativo").map(e => `<option value="${e.id}" ${np.employeeId === e.id ? "selected" : ""}>${U.esc(e.name)}</option>`).join("")}
          </select>
        </div>
        <div class="field"><label>Máquina</label>
          <select data-npo-k="machineId" data-order-id="${o.id}">
            <option value="">Selecione...</option>
            ${machines.map(m => `<option value="${m.id}" ${np.machineId === m.id ? "selected" : ""}>${U.esc(m.name)}</option>`).join("")}
          </select>
        </div>
        <div class="field"><label>Produto a produzir</label><input data-npo-k="product" data-order-id="${o.id}" value="${U.esc(np.product)}" placeholder="Ex.: Miolo 160 folhas"></div>
        <div class="field-row">
          <div class="field"><label>Quantidade planejada</label><input type="number" data-npo-k="quantityPlanned" data-order-id="${o.id}" min="0" value="${np.quantityPlanned}"></div>
          <div class="field"><label>Operação</label>
            <select data-npo-k="operacao" data-order-id="${o.id}">
              <option value="producao" ${np.operacao === "producao" ? "selected" : ""}>Produção</option>
              <option value="retrabalho" ${np.operacao === "retrabalho" ? "selected" : ""}>Retrabalho</option>
            </select>
          </div>
        </div>
        <button class="btn btn-primary btn-block" data-act="save-new-po" data-id="${o.id}">Adicionar ordem</button>
      </div>`;
  }
  ...
  async function addProductionOrder(orderId) {
    const np = state.newPO[orderId] || {};
    const m = machines.find(x => x.id === np.machineId);
    const product = (np.product || "").trim();
    const qty = Number(np.quantityPlanned) || 0;
    if (!m) { toast("Selecione a máquina."); return; }
    if (!np.employeeId) { toast("Selecione o funcionário responsável."); return; }
    const employee = employees.find(e => e.id === np.employeeId);
    if (!employee || employee.status !== "ativo") { toast("Funcionário inválido ou inativo."); return; }
    if ((employee.allowedMachines || []).indexOf(m.id) < 0) { toast("Esse funcionário não tem permissão para operar essa máquina."); return; }
    if (!product) { toast("Informe o produto a produzir."); return; }
    if (qty <= 0) { toast("Informe a quantidade planejada."); return; }
    await S.add("productionOrders", {
      orderId,
      machineId: m.id,
      machineName: m.name,
      employeeId: np.employeeId || "",
      employeeName: np.employeeId ? ((employees.find(e => e.id === np.employeeId) || {}).name || "") : "",
      product,
      quantityPlanned: qty,
      operacao: np.operacao === "retrabalho" ? "retrabalho" : "producao",
      status: "aguardando",
      quantityProduced: 0,
      defects: 0,
      startedAt: null,
    ...
    state.newPOForm[orderId] = false;
    delete state.newPO[orderId];
    toast("Ordem de produção criada");
  }
  ...
  function bindAllExtra() {
    ...
    document.querySelectorAll("[data-npo-k]").forEach(el => {
      const oid = el.getAttribute("data-order-id");
      if (!state.newPO[oid]) state.newPO[oid] = { machineId: "", employeeId: "", product: "", quantityPlanned: "", operacao: "producao" };
      const k = el.getAttribute("data-npo-k");
      const ev = el.tagName === "SELECT" ? "change" : "input";
      el.addEventListener(ev, () => { state.newPO[oid][k] = el.value; });
    });
  }
  ...
  function onClickGlobal(e) {
    ...
    if (a === "login") doLogin();
      ...
      if (confirm("Excluir este lançamento?")) S.remove("productions", lid);
    }
    ...
    else if (a === "new-po") { state.newPOForm[id] = true; state.newPO[id] = { machineId: "", employeeId: "", product: "", quantityPlanned: "", operacao: "producao" }; renderView(); }
    else if (a === "cancel-new-po") { state.newPOForm[id] = false; renderView(); }
    else if (a === "save-new-po") addProductionOrder(id);
