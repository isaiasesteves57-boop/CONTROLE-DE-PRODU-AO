...
(function () {
  ...
  const freshOPDraft = () => ({ orderId: "", quantity: "", priority: "media" });
  ...
  const freshTaskDraft = () => ({ orderId: "", opId: "", employeeId: "", machineId: "", productId: "", model: "" });
  ...
  try {
    ...
    if (["dashboard", "industria", "relatorios", "planejamento", "pedidos", "ops", "estoque", "produtividade", "qualidade", "maquinas", "produtos", "funcionarios", "config"].indexOf(h) >= 0) state.page = h;
  } catch (e) { }
  ...
  */
  ...
  /* Código curto e estável da OP, mesma lógica do orderCode(). */
  function opCode(op) {
      ...
      <div class="card pad" style="background:var(--panel)">
        <div class="rowc">
          <div class="f1">
            ...
            <div class="tiny dim">${order ? U.esc(order.product) + (order.model ? " — " + U.esc(order.model) : "") : "Pedido não encontrado"}</div>
          </div>
  ...
  function renderOPDetalhe(v) {
    ...
    v.innerHTML = `
      ...
      <div class="card pad">
        ...
        <div class="kv mt12"><b>Tempo total:</b> ${tempoTotal}</div>
    ...
    return `
      <div class="card pad">
        ...
        <div class="field"><label>Pedido</label>
          <select id="nop-order">
            <option value="">Selecione...</option>
            ${pending.map(x => `<option value="${x.id}" ${no.orderId === x.id ? "selected" : ""}>${orderCode(x)} — ${U.esc(x.client)} — ${U.fmt(x._remaining)} pendentes — ${U.esc(x.product)}</option>`).join("")}
          </select>
