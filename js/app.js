...
(function () {
  ...
  const freshOPDraft = () => ({ orderId: "", number: "", sector: "", dueDate: "", quantity: "", priority: "media" });
  ...
  */
  ...
  function opCode(op) {
    if (op && op.number != null && String(op.number).trim() !== "") return String(op.number).trim();
      ...
      <div class="card pad" style="background:var(--panel)">
        <div class="rowc">
          <div class="f1">
            ...
            <div class="tiny dim">${order ? U.esc(order.product) + (order.model ? " — " + U.esc(order.model) : "") : "Pedido não encontrado"}${op.sector ? " · " + U.esc(op.sector) : ""}${op.dueDate ? " · prev.: " + U.fmtDate(op.dueDate) : ""}</div>
          </div>
          <span class="pill ${prOP.pill}">${prOP.label}</span>
          <span class="pill ${stOP.pill}">${stOP.label}</span>
        </div>
        <div class="kv mt12"><b>Planejado:</b> ${qty ? U.fmt(qty) + " un." : "—"}</div>
      ...
      <div class="card pad">
        <div class="rowc">
          <div class="f1">
            ...
            <div class="tiny dim">${order ? U.esc(order.client) + " · " + U.esc(order.product) + (order.model ? " — " + U.esc(order.model) : "") : "Pedido não encontrado"}</div>
          </div>
          <span class="pill ${prOP.pill}">${prOP.label}</span>
          <span class="pill ${stOP.pill}">${stOP.label}</span>
        </div>
    ...
    return `
      <div class="card pad">
        ...
        <div class="field"><label>Número da OP (manual)</label>
          ...
          <p class="tiny dim mt8" id="nop-number-err">Cada número é único — o sistema impede duplicar.</p>
        </div>
        <div class="field-row">
  ...
  async function addOP() {
    ...
    if (ops.some(x => String(x.number || "").trim() === numero)) {
  ...
  function bindAllExtra() {
    ...
    if (nopOrder) nopOrder.addEventListener("change", () => { state.newOP.orderId = nopOrder.value; renderView(); });
    ...
    if (nopNumber) {
      nopNumber.addEventListener("input", () => {
        ...
        if (errEl) {
          const dup = ops.some(x => String(x.number || "").trim() === String(nopNumber.value).trim());
          const dupShown = String(state.newOP.number).trim() !== "" && dup;
          errEl.style.color = dupShown ? "var(--danger)" : "";
          errEl.textContent = dupShown
            ? "Esta OP já existe. Informe outro número."
            : "Cada número é único — o sistema impede duplicar.";
        }
      });
    }
