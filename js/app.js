...
(function () {
  ...
  const freshOrderDraft = () => ({ number: "", client: "", description: "", product: "", sheets: "", type: "", model: "", finish: "", quantity: "" });
  ...
  let machines = [], products = [], productions = [], employees = [], orders = [], productionOrders = [], ops = [], materials = [], stockMovements = [], defectRecords = [], auditLogs = [], config = SEED_CONFIG[0];
  ...
  */
  ...
  function orderCard(o) {
    ...
    return `
      <div class="card pad"><div class="rowc"><div class="f1">
        ...
        <div class="tiny dim">${o.description?U.esc(o.description)+" · ":""}${U.esc(o.product)}${o.sheets?" · "+U.esc(String(o.sheets))+" folhas":""}${o.type?" · "+U.esc(o.type):""}${o.model?" · "+U.esc(o.model):""}${o.finish?" · "+U.esc(o.finish):""}</div></div>
    ...
    return `
      <div class="card pad">
        ...
        <div class="field"><label>Descrição</label>
          <select id="no-description" required>
            <option value="">Selecione...</option>
        ...
        <div class="field-row">
          <div class="field"><label>Modelo</label>
            <select id="no-model" required>
              <option value="">Selecione...</option>
              ${modelOptions}
            </select>
          </div>
          <div class="field"><label>Acabamento</label>
            <select id="no-finish" required>
              <option value="">Selecione...</option>
              ${finishOptions}
            </select>
          </div>
  ...
  function renderHistorico(v) {
    ...
    v.innerHTML = `
      <div class="space-y">
        ...
        <div class="tiny dim mt8">Registro automático de edições e exclusões feitas pelo gestor.</div></div>
  ...
  async function addOrder() {
    ...
    if (!description || !product || !sheets || !type || !model || !finish || quantity <= 0) {
      toast("Preencha descrição, produto, quantidade de folhas, tipo, modelo, acabamento e uma quantidade maior que zero.");
      return;
    }
  ...
  async function saveOrder(id) {
    ...
    const patch={client:String(d.client||"").trim(),number:String(d.number||"").trim(),description,product,sheets,type:String(d.type||"").trim(),model:String(d.model||"").trim(),finish:String(d.finish||"").trim(),quantity:Number(d.quantity)||0};
    const allowed=ORDER_PRODUCTS_BY_DESCRIPTION[description]||[];
    if(!patch.client||patch.quantity<=0||!description||!allowed.includes(product)||!sheets||!ORDER_SHEET_OPTIONS.includes(sheets)||!ORDER_TYPES.includes(patch.type)||!ORDER_MODELS.includes(patch.model)||!ORDER_FINISHES.includes(patch.finish)){toast("Revise os campos do pedido. Produto deve ser compatível com a descrição e a quantidade de folhas deve ser selecionada.");return;}
  ...
  function onClickGlobal(e) {
    ...
    if (a === "login") doLogin();
      ...
      if (o) state.draftOrder[id] = {
        client: o.client, number: o.number || "", description: o.description || "", product: o.product || "",
