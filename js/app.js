...
(function () {
  ...
  const freshTaskDraft = () => ({ orderId: "", opId: "", employeeId: "", machineId: "", productId: "", model: "", plannedQuantity: "", estimatedMinutes: "", priority: "media", observation: "" });
  ...
  function init() {
    ...
    root.addEventListener("dragstart", e => { const el = e.target.closest("[data-queue-task]"); if (el && e.dataTransfer) e.dataTransfer.setData("text/plain", el.getAttribute("data-queue-task")); });
    root.addEventListener("dragover", e => { if (e.target.closest("[data-queue-task]")) e.preventDefault(); });
    root.addEventListener("drop", async e => {
      const target = e.target.closest("[data-queue-task]"); if (!target) return; e.preventDefault();
      const fromId = e.dataTransfer ? e.dataTransfer.getData("text/plain") : ""; const toId = target.getAttribute("data-queue-task");
      if (!fromId || fromId === toId) return;
      const empId = state.selectedEmployeeId; const list = employeeOpenTasks(empId); const from = list.findIndex(x=>x.id===fromId), to = list.findIndex(x=>x.id===toId);
      if (from<0 || to<0) return; const [item] = list.splice(from,1); list.splice(to,0,item);
      try { await FluxoProducao.reordenarFilaFuncionario(empId, list.map(x=>x.id)); renderView(); } catch(err) { toast(errMsg(err)); }
    });
    renderAll();
  }
  ...
  */
  ...
  function renderPedidos(v) {
    v.innerHTML = `
      <div class="space-y">
        ${state.newOPForm ? newOPForm() : (state.newTaskForm ? newTaskForm() : `<button class="btn btn-primary btn-block" data-act="new-task">➕ Nova Tarefa</button>`)}
    ...
    return `
      <div class="card pad">
        ...
        <div class="rowc mt12" style="gap:8px;flex-wrap:wrap">
          ...
          ${remaining > 0 ? `<button class="btn-ghost" data-act="new-task-for-order" data-id="${o.id}">➕ Tarefa avulsa (sem OP)</button>` : ""}
        </div>
    ...
    return `
      <div class="card pad" style="background:var(--panel)">
        ...
        <div class="rowc mt12" style="gap:8px;flex-wrap:wrap">
          ...
          <button class="btn-ghost" data-act="new-task-for-op" data-id="${op.id}">➕ Nova tarefa</button>
  ...
  function newTaskForm() {
    ...
    const pending = ordersWithBalance();
    const order = op ? (orders.find(o => o.id === op.orderId) || null) : (pending.find(o => o.id === nt.orderId) || null);
    const orderRemaining = order ? Math.max(0, (Number(order.quantity) || 0) - orderProgress(order.id).produced) : null;
    ...
    return `
      <div class="card pad">
        ...
        ${op ? `
        <div class="kv" style="margin-top:0"><b>Ordem de Produção:</b> ${opCode(op)}${order ? " — " + U.esc(order.client) + " — " + U.esc(order.product) : ""}</div>
        ` : `
        <div class="field"><label>Pedido</label>
          <select id="nt-order">
            <option value="">Selecione...</option>
            ${pending.map(o => `<option value="${o.id}" ${nt.orderId === o.id ? "selected" : ""}>${o.number ? "Pedido " + U.esc(o.number) + " · " : ""}${orderCode(o)} — ${U.esc(o.client)} — ${U.fmt(o._remaining)} pendentes — ${U.esc(o.product)}</option>`).join("")}
          </select>
          ${!pending.length ? '<p class="tiny dim mt8">Nenhum pedido com saldo pendente. Cadastre um pedido primeiro.</p>' : ""}
        </div>`}
        ...
        ${order ? `<div class="kv mt12"><b>Quantidade disponível do pedido:</b> ${U.fmt(orderRemaining)} un.</div>` : ""}

        <div class="field-row">
          <div class="field"><label>Quantidade planejada (un.)</label><input type="number" id="nt-planned" min="0" value="${nt.plannedQuantity}" placeholder="${orderRemaining != null ? U.fmt(orderRemaining) : "Ex.: 5000"}"></div>
          <div class="field"><label>Tempo estimado (min)</label><input type="number" id="nt-est" min="0" value="${nt.estimatedMinutes}" placeholder="Ex.: 240"></div>
        </div>
        <div class="field-row">
          <div class="field"><label>Prioridade</label>
            <select id="nt-priority">
              <option value="baixa" ${nt.priority === "baixa" ? "selected" : ""}>Baixa</option>
              <option value="media" ${nt.priority === "media" ? "selected" : ""}>Média</option>
              <option value="alta" ${nt.priority === "alta" ? "selected" : ""}>Alta</option>
            </select>
          </div>
          <div class="field"><label>Observação</label><input type="text" id="nt-obs" value="${U.esc(nt.observation)}" placeholder="Opcional"></div>
        </div>
  ...
  async function addOrder() {
    ...
    try {
      await S.add("orders", {
        client, product,
        number: (state.newOrder.number || "").trim(),
  ...
  async function doAddTask() {
    ...
    const planned = Number(nt.plannedQuantity) > 0 ? Number(nt.plannedQuantity) : remaining;
    if (planned > remaining) {
      toast("A quantidade planejada é maior que o saldo do pedido.");
      return;
    }
    try {
      await S.add("productionOrders", {
        ...
        estimatedMinutes: Number(nt.estimatedMinutes) || 0,
        priority: nt.priority || "media",
        ...
        finishedAt: null,
        queuePosition: (() => {
          const q = productionOrders.filter(x => x.employeeId === employee.id && x.status !== "finalizada");
          return q.length ? Math.max(...q.map(x => Number(x.queuePosition) || 0)) + 1 : 1;
        })(),
        createdAt: Date.now()
      });
  ...
  function employeeCard(em) {
    ...
    return `
      <div class="card pad">
        ...
        <div class="rowc mt12" style="gap:8px;flex-wrap:wrap">
          <button class="btn-ghost" data-act="open-emp-detail" data-id="${em.id}">📊 Ver detalhes</button>
          <button class="btn-ghost" data-act="toggle-emp-status" data-id="${em.id}">${active ? "Desativar" : "Ativar"}</button>
        </div>
      </div>`;
  }
  ...
  function currentTaskForEmployee(employeeId) {
    const active = productionOrders.find(po => po.employeeId === employeeId && (po.status === "em_producao" || po.status === "pausa"));
    if (active) return active;
    return productionOrders.filter(po => po.employeeId === employeeId && po.status !== "finalizada")
      .slice().sort((a, b) => (Number(a.queuePosition) || 0) - (Number(b.queuePosition) || 0) || (a.createdAt || 0) - (b.createdAt || 0))[0] || null;
  }

  /* Todas as tarefas em aberto do funcionário (não só a mais recente),
     usada no detalhe avançado (Etapa 5). */
  ...
  async function moveEmployeeTask(employeeId, taskId, direction) {
    const list = employeeOpenTasks(employeeId);
    const i = list.findIndex(x => x.id === taskId);
    if (i < 0) return;
    const j = direction === "up" ? i - 1 : i + 1;
    if (j < 0 || j >= list.length) return;
    [list[i], list[j]] = [list[j], list[i]];
    for (let k = 0; k < list.length; k++) await S.update("productionOrders", list[k].id, { queuePosition: k + 1 });
    renderView();
  }

  async function urgentEmployeeTask(taskId) {
    if (!window.FluxoProducao) return;
    try { await FluxoProducao.priorizarTarefa({ taskId }); toast("Tarefa colocada como próxima da fila"); renderView(); }
    catch (e) { toast(errMsg(e)); }
  }

  async function interruptForUrgent(taskId) {
    const urgent = productionOrders.find(x => x.id === taskId);
    if (!urgent) return;
    const current = productionOrders.find(x => x.employeeId === urgent.employeeId && x.status === "em_producao");
    try {
      if (current) await S.update("productionOrders", current.id, { status: "pausa", pausedAt: Date.now() });
      await FluxoProducao.priorizarTarefa({ taskId });
      await S.update("productionOrders", urgent.id, { priority: "alta", queuePosition: 0 });
      await FluxoProducao.iniciarTarefa({ taskId: urgent.id });
      toast(current ? "Tarefa atual pausada e urgente iniciada" : "Tarefa urgente iniciada");
      renderView();
    } catch (e) { toast(errMsg(e)); }
  }
  ...
  function renderFuncionarioDetalhe(v) {
    ...
    if (!em) {
      v.innerHTML = `<div class="empty">Funcionário não encontrado.</div><button class="btn-ghost mt12" data-go="funcionarios">← Voltar para Funcionários</button>`;
      return;
    }
    const st = employeeStats(em.id);
    const ranking = employeeRanking();
    const rankIdx = ranking.findIndex(x => x.id === em.id);
    const teamAvgHour = ranking.length ? ranking.reduce((s, x) => s + x.avgHour, 0) / ranking.length : 0;
    const task = currentTaskForEmployee(em.id);
    const taskMachine = task ? machines.find(m => m.id === task.machineId) : null;
    const taskOP = task && task.opId ? ops.find(x => x.id === task.opId) : null;
    const openTasks = employeeOpenTasks(em.id);

    const days14 = [];
    for (let i = 13; i >= 0; i--) days14.push(addDaysDS(U.todayStr(), -i));
    ...
    const byProduct = {};
    st.rows.forEach(r => { const name = r.productName || "—"; byProduct[name] = (byProduct[name] || 0) + (Number(r.quantityProduced) || 0); });
    const prodEntries = Object.entries(byProduct).sort((a, b) => b[1] - a[1]).slice(0, 6);

    const byMachine = {};
    st.rows.forEach(r => { const name = r.machineName || "—"; byMachine[name] = (byMachine[name] || 0) + (Number(r.quantityProduced) || 0); });
    const machEntries = Object.entries(byMachine).sort((a, b) => b[1] - a[1]).slice(0, 6);
    ...
    v.innerHTML = `
      ...
      <div class="card pad">
        <div class="rowc">
          ...
          <div class="f1">
            <div class="small" style="font-weight:700;font-size:16px">${U.esc(em.name)}</div>
            <div class="tiny dim">${U.esc(em.role || "—")}${config.sector ? " · " + U.esc(config.sector) : ""}</div>
          </div>
          ${rankIdx >= 0 ? `<span class="pill good">🏆 Ranking #${rankIdx + 1}</span>` : ""}
        </div>
        <div class="kv mt12"><b>Máquina atual:</b> ${taskMachine ? U.esc(taskMachine.name) : "— (sem tarefa em aberto)"}</div>
        <div class="kv"><b>OP atual:</b> ${taskOP ? opCode(taskOP) : (task ? "Tarefa avulsa (sem OP)" : "—")}</div>
        ${task ? `<div class="kv"><b>Produto atual:</b> ${U.esc(task.product || "")}${task.model ? " — " + U.esc(task.model) : ""}</div>` : ""}
      </div>

      <div class="grid-cards mt12">
        <div class="card stat"><div class="lbl">${U.icon("box", "sm")} Produtos produzidos</div><div class="val">${U.fmt(st.productCount)}</div><div class="foot">tipos diferentes</div></div>
        <div class="card stat accent"><div class="lbl">${U.icon("package", "sm")} Quantidade total</div><div class="val amb">${U.fmt(st.total)}<em>un.</em></div><div class="foot">${U.fmt(st.dayCount)} dia${st.dayCount !== 1 ? "s" : ""} trabalhado${st.dayCount !== 1 ? "s" : ""}</div></div>
        <div class="card stat"><div class="lbl">${U.icon("calendar", "sm")} Média diária</div><div class="val">${U.fmt(st.avgDay)}<em>un.</em></div><div class="foot">un. por dia trabalhado</div></div>
        <div class="card stat"><div class="lbl">${U.icon("speed", "sm")} Média hora</div><div class="val">${U.fmt(st.avgHour, 1)}<em>un./h</em></div><div class="foot">${U.fmt(st.hours, 2)} h no total</div></div>
        <div class="card stat"><div class="lbl">${U.icon("checkc", "sm")} Melhor produção</div><div class="val up">${U.fmt(st.best, 1)}<em>un./h</em></div><div class="foot">melhor lançamento</div></div>
        <div class="card stat"><div class="lbl">${U.icon("alert", "sm")} Pior produção</div><div class="val ${st.worst ? "down" : ""}">${st.worst ? U.fmt(st.worst, 1) : "—"}<em>${st.worst ? "un./h" : ""}</em></div><div class="foot">pior lançamento</div></div>
      </div>

      <div class="card pad mt12">
        <span class="section-label" style="margin:0">${U.icon("speed", "sm")} Comparação com a equipe</span>
        <div class="grid3 mt12">
          <div class="plan-kpi"><div class="pk">Sua média</div><div class="pv">${U.fmt(st.avgHour, 1)}</div><div class="tiny dim mt8">un./h</div></div>
          <div class="plan-kpi"><div class="pk">Média da equipe</div><div class="pv">${teamAvgHour ? U.fmt(teamAvgHour, 1) : "—"}</div><div class="tiny dim mt8">un./h</div></div>
          <div class="plan-kpi"><div class="pk">Diferença</div><div class="pv" style="font-size:16px">${teamAvgHour ? i4Trend(st.avgHour, teamAvgHour) : "—"}</div><div class="tiny dim mt8">vs. equipe</div></div>
        </div>
      </div>

      <div class="i4-charts-row mt12">
        <div class="card pad i4-chart-wide">
          <span class="section-label" style="margin:0 0 8px">${U.icon("chart", "sm")} Evolução da produção <span class="tiny dim" style="text-transform:none;font-weight:400">(últimos 14 dias)</span></span>
          ${seriesDaily.some(x => x > 0) ? lineChartHTML(dayLbls14, seriesDaily, { color: "var(--cyan)" }) : '<div class="empty">Sem produção registrada nos últimos 14 dias.</div>'}
        </div>
        <div class="card pad">
          <span class="section-label" style="margin:0 0 8px">${U.icon("box", "sm")} Produtos mais produzidos</span>
          ${prodEntries.length ? donutMultiHTML(prodEntries) : '<div class="empty">Sem dados de produção.</div>'}
        </div>
        <div class="card pad">
          <span class="section-label" style="margin:0 0 8px">${U.icon("cog", "sm")} Máquinas mais utilizadas</span>
          ${machEntries.length ? barChartHTML(machEntries.map(e => e[0]), machEntries.map(e => e[1]), { color: "var(--ok)" }) : '<div class="empty">Sem dados de produção.</div>'}
        </div>
      </div>

      <div class="rowc between mt12">
        <p class="section-label" style="margin:0">${U.icon("clipboard", "sm")} FILA DE ${U.esc(em.name.toUpperCase())} (${openTasks.length})</p>
        <button class="btn btn-primary" data-act="new-task-for-employee" data-id="${em.id}">➕ Adicionar tarefa</button>
      </div>
      <p class="tiny dim mt8">Arraste para mudar a sequência. A tarefa atual mantém o progresso; ao concluir, a próxima assume.</p>
      <div class="space-y3 mt8" data-employee-queue="${em.id}">
        ${openTasks.length ? openTasks.map((po, idx) => {
          const opRef = po.opId ? ops.find(x => x.id === po.opId) : null;
          const planned = Number(po.plannedQuantity) || 0, produced = Number(po.quantityProduced) || 0, remaining = Math.max(0, planned-produced);
          const active = po.status === "em_producao" || po.status === "pausa";
          return `<div draggable="true" data-queue-task="${po.id}" class="card pad" style="background:var(--panel)">
            <div class="rowc between"><div><b>${idx+1}. ${U.esc(po.product || "Tarefa")}</b><div class="tiny dim">${opRef ? opCode(opRef) : "Tarefa avulsa"} · ${U.esc(po.machineName || "")} · ${active ? (po.status === "pausa" ? "PAUSADA" : "EM PRODUÇÃO") : "AGUARDANDO"}</div></div><span class="pill ${active ? "warn" : ""}">${planned ? U.fmt(remaining)+" restantes" : "—"}</span></div>
            <div class="rowc mt8" style="gap:6px;flex-wrap:wrap">
              <button class="btn-ghost" data-act="queue-up" data-id="${po.id}">↑ Subir</button>
              <button class="btn-ghost" data-act="queue-down" data-id="${po.id}">↓ Descer</button>
              <button class="btn-ghost" data-act="queue-urgent" data-id="${po.id}">🚨 Próxima</button>
              ${!active ? `<button class="btn-ghost" data-act="queue-interrupt" data-id="${po.id}">⚡ Iniciar agora</button>` : ""}
            </div>
          </div>`;
        }).join("") : '<div class="empty">Nenhuma tarefa em aberto no momento.</div>'}
      </div>

      <div class="card pad mt12">
        <span class="section-label" style="margin:0">${U.icon("chart", "sm")} Histórico de produção</span>
        ${st.rows.length ? `
        <div class="card tbl-wrap mt12">
          <table>
            <thead><tr>
              <th>Data</th><th>OP</th><th>Produto</th><th>Máquina</th>
              <th style="text-align:right">Horas</th><th style="text-align:right">Produção</th>
              <th style="text-align:right">Aprovado</th><th style="text-align:right">Defeito</th>
              <th style="text-align:right">Prod/h</th>
            </tr></thead>
            <tbody>
              ${st.rows.map(r => {
                const po = r.productionOrderId ? productionOrders.find(x => x.id === r.productionOrderId) : null;
                const op = po && po.opId ? ops.find(x => x.id === po.opId) : null;
                const qty = Number(r.quantityProduced) || 0;
                const def = Number(r.defects) || 0;
                return `<tr>
                  <td>${U.fmtDate(r.date)}</td>
                  <td>${op ? opCode(op) : "—"}</td>
                  <td>${U.esc(r.productName || "—")}${r.model ? " — " + U.esc(r.model) : ""}</td>
                  <td>${U.esc(r.machineName || "—")}</td>
                  <td style="text-align:right">${U.fmt(r.productionHours, 2)}</td>
                  <td style="text-align:right">${U.fmt(qty)}</td>
                  <td style="text-align:right">${U.fmt(qty - def)}</td>
                  <td style="text-align:right">${def ? U.fmt(def) : "—"}</td>
                  <td style="text-align:right">${U.fmt(r.perHour, 1)}</td>
                </tr>`;
              }).join("")}
            </tbody>
          </table>
        </div>` : '<div class="empty mt12">Nenhum apontamento registrado para este funcionário ainda.</div>'}
      </div>`;
  }
  ...
  function onClickGlobal(e) {
    ...
    else if (a === "new-task-for-employee") { state.newTaskForm = true; state.newTask = freshTaskDraft(); state.newTask.employeeId = id; go("pedidos"); }
    else if (a === "queue-up" || a === "queue-down") { moveEmployeeTask(state.selectedEmployeeId, id, a === "queue-up" ? "up" : "down"); }
    else if (a === "queue-urgent") urgentEmployeeTask(id);
    else if (a === "queue-interrupt") interruptForUrgent(id);
    ...
    else if (a === "new-task-for-op") {
      const op = ops.find(x => x.id === id);
      state.newTaskForm = true;
      state.newTask = freshTaskDraft();
      if (op) {
        state.newTask.opId = op.id;
