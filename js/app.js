  ...
  */
        ...
        ${machines.map(m => {
          ...
          return `
            <div class="card pad">
              ...
              <div class="mt12">
                <div class="tiny dim" style="letter-spacing:.08em;text-transform:uppercase">Fila completa</div>
                <div class="space-y3 mt8">
                  ${[...(fila.atual ? [fila.atual] : []), ...fila.fila].map((item, idx) => {
                    const ii = taskFilaInfo(item);
                    const isAtual = fila.atual && item.id === fila.atual.id;
                    return `<div class="card" style="background:var(--panel);padding:10px">
                      <div class="rowc">
                        <div class="f1">
                          <div class="small" style="font-weight:600">${isAtual ? "▶ " : (idx + 1) + ". "}${U.esc(ii.opCode || ii.product)}</div>
                          <div class="tiny dim">${U.esc(ii.product)}${ii.employee ? " · " + U.esc(ii.employee) : ""} · ${isAtual ? (item.status === "pausa" ? "PAUSADA" : "EM PRODUÇÃO") : "AGUARDANDO"}</div>
                          <div class="tiny dim">${U.fmt(ii.produced)} / ${U.fmt(ii.planned)} · restante ${U.fmt(ii.remaining)}</div>
                        </div>
                        ${item.priority === "alta" ? '<span class="pill parada">URGENTE</span>' : ''}
                        ${!isAtual && item.status === "aguardando" ? `<button class="btn-ghost" data-act="fila-up" data-id="${item.id}">↑</button><button class="btn-ghost" data-act="fila-down" data-id="${item.id}">↓</button><button class="btn-ghost" data-act="fila-urgent" data-id="${item.id}">🚨</button>` : ''}
                      </div>
                    </div>`;
                  }).join("")}
                  ${!fila.atual && !fila.fila.length ? '<div class="empty">Fila vazia.</div>' : ''}
                </div>
              </div>
  ...
  function currentTaskForEmployee(employeeId) {
    return productionOrders
      ...
      .slice().sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))[0] || null;
  }
  ...
  function renderFuncionarioDetalhe(v) {
    ...
    v.innerHTML = `
      ...
      <p class="section-label">${U.icon("clipboard", "sm")} Tarefas em aberto (${openTasks.length})</p>
      <div class="space-y3">
        ${openTasks.length ? openTasks.map(po => {
          const opRef = po.opId ? ops.find(x => x.id === po.opId) : null;
          return `<div class="tiny dim" style="margin-bottom:-4px">${opRef ? opCode(opRef) : "Tarefa avulsa (sem OP)"}</div>${poCard(po)}`;
  ...
  function onClickGlobal(e) {
    ...
    else if (a === "fila-up") { FluxoProducao.moverFila(actWrap.getAttribute("data-id"), "up").then(() => renderView()).catch(e => toast(errMsg(e))); }
    else if (a === "fila-down") { FluxoProducao.moverFila(actWrap.getAttribute("data-id"), "down").then(() => renderView()).catch(e => toast(errMsg(e))); }
    else if (a === "fila-urgent") { FluxoProducao.definirUrgencia(actWrap.getAttribute("data-id")).then(() => { toast("Tarefa colocada como URGENTE e no topo da fila."); renderView(); }).catch(e => toast(errMsg(e))); }
